...
def insert_feature_stats(mu: Sequence[float], n_recipes: int, note: str) -> int:
    ...
    with cursor(commit=True) as cur:
        ...
        row = cur.fetchone()
        if row is None:
            raise RuntimeError("feature_stats INSERT 가 id 를 돌려주지 않았습니다")
        return int(row[0])
...
def load_popularity_stats() -> tuple[float, float, float, int]:
    ...
    with cursor() as cur:
        ...
        # 집계 질의라 행이 정확히 하나다
        return cast("tuple[float, float, float, int]", cur.fetchone())
...
def start_batch_run(job_name: str, params: str | None = None) -> int:
    ...
    with cursor(commit=True) as cur:
        ...
        row = cur.fetchone()
        if row is None:
            raise RuntimeError("batch_run INSERT 가 id 를 돌려주지 않았습니다")
        return int(row[0])
...
def load_gate_counts() -> tuple[int, ...]:
    ...
    with cursor() as cur:
        ...
        # 집계 질의라 행이 정확히 하나다
        return cast("tuple[int, ...]", cur.fetchone())
