...
**버전**: 2.1.0 · **최종 수정**: 2026-09-11 · **작성자**: 유재현
...
**위 식은 2026-09-11 2차 회의로 대체됐습니다.** 현재 구현은 고른 음식의 6축 평균을 사전 취향으로, 시각이 있는 이벤트를 반감기 감쇠와 주기 친화도로 가중 평균해 `(k·p + S·b)/(k + S)` 로 합칩니다. 근거는 `../decisions/2026-09-11_taste_persona_from_picks_with_time_decay.md`, 코드는 `engine/persona.py` 입니다.
