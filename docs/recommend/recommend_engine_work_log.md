...
**버전**: 1.33.0 · **최종 수정**: 2026-09-23 · **작성자**: 유재현
...
| 브랜치 | **`main` 이 최신입니다** — PR #17(`c8dcaf2`, 09-22 18:11) · #18(`6bfc6a8`) · **#19(`356be83`, 2026-09-23 09:04)** 까지 병합. 병합이 곧 배포이므로 그 시점부터 `main` 의 코드가 운영입니다 — 클라우드가 `BACKEND_BASE_URL` 을 넣기 전까지는 목업으로 답합니다. 데이터 파트의 `develop-data-part` 는 그쪽이 병합합니다 |
...
| 다음 행동 | **남은 일의 정본은 `recommend_remaining_work.md` 입니다**(2026-09-22). 다른 파트를 기다리지 않는 것 — W-05(조리 이력 → 두 신호) · W-07(오프라인 평가 스크립트) · W-03(이미지 재빌드). `main` 병합(W-01)은 클라우드의 배포 설정(C-01~C-04)이 들어간 뒤, 유재현의 지시로 |
...
| D-52 | 같은 점수는 `recipe_id` 순 | `score.tie_break(user_id, recipe_id)` 의 안정 해시 순. 정렬 키 `score.order_key` 하나를 `score_all` 과 `rerank.rerank` 가 같이 씁니다 | 실데이터에서 개인화 경계에 9~14건이 동점으로 걸려 번호가 낮은 레시피가 모든 사용자에게 언제나 이겼습니다(F-119). 난수는 TC-5-1 을, 파이썬 `hash()` 는 로그 재현을 깹니다 | 인기도(G-34)가 오면 동점 자체가 드물어집니다. 그때도 규칙은 그대로 둡니다 — 동점이 0 이 되지는 않습니다 |
| D-53 | 알레르기는 영문 군 코드 10종 | 한글 라벨을 받아 **시드의 군 전체**를 막습니다(땅콩 → 견과 전부 · 새우 → 조개류·김치까지). 영문 코드도 같은 함수가 받고, 모르는 라벨은 버리지 않고 돌려줍니다(`engine/allergy.py`) | 백엔드가 `["우유", "땅콩"]` 모양으로 보냅니다(09-21 유재현). 덜 막는 쪽과 더 막는 쪽의 무게가 다릅니다 | 백엔드 재료 사전에 `allergens` 배열이 생기면 라벨별로 정밀하게 좁힙니다 |
| D-54 | 하드컷은 재료 id 의 교집합 | 재료 id **와 제목**을 함께 봅니다(`allergy.blocks`). 제목에서 찾는 말은 막힌 재료의 이름과 다른 말입니다. 한 글자 조각은 넣지 않습니다 | 실데이터에 재료 행이 빠진 레시피가 있습니다 — 새우 알레르기 사용자의 1위가 「고추장 건새우볶음」 이었습니다(F-121). "노밀가루" 같은 부정 표현은 읽지 않고 막습니다 | 백엔드가 빠진 재료 행을 복구하면 제목 검사를 좁힐 수 있습니다. 없애지는 않습니다 |
| D-55 | 검증 실패는 400 · 빈 본문 (영수증 파트가 앱 전체에 건 핸들러, 09-07) | **영수증 경로(`/v1/ocr`)만 빈 본문**이고 나머지는 `ValidationErrorOut`(`error` · `fields[]` 의 `location` · `field` · `code` · `message`)을 싣습니다. pydantic 의 `input` 은 싣지 않습니다 | 백엔드 요청입니다(09-21, F-125). 추천 계약은 모르는 필드 하나로 요청 전체를 거부하므로 본문이 비면 백엔드가 원인을 알 수 없습니다. 영수증 계약은 빈 본문을 검사로 박아 두었으므로(`test_missing_field_is_400_not_422`) 건드리지 않고 경로로 갈랐습니다. 보낸 값을 되돌려주면 요청 본문이 응답과 호출 쪽 로그로 복사됩니다 |
...
| D-62 | 요청의 `max_missing` 은 부족 재료의 상한 (목업은 그 값으로 딱 자름) | 실서빙에서는 **사다리의 시작값**입니다. 후보가 모자라면 거기서부터 4(요청이 더 크면 그 값)까지 풀고 그래도 모자라면 인기순입니다 | 엔진의 설계(A-10)가 "모자라면 푼다" 이고, 딱 자르면 재료가 적은 냉장고에서 빈 목록이 나갑니다. 풀었는지는 추적의 `fallback` 과 항목의 `missing_count` 로 보입니다. 명세 4.4 에 그대로 적었습니다 | 백엔드가 "지금 만들 수 있는 것만" 같은 엄격한 화면을 만들면서 빈 목록을 감수하겠다고 하면 요청에 엄격 표시를 더하는 것을 논의합니다 |
| D-63 | 사용자 이력은 A 의 표(`user_ingredient_pref` · `event_log` · `user_cluster_stat`)에서 읽음 (M-03 · T-18) | **우리 취향 이벤트와 메모리 사전에서 만듭니다**(`engine/history.py`). 조리 14일 · 노출 7일 창, 선호 재료는 종류 무게 x 시간 감쇠(취향 벡터와 같은 반감기) 합이 1.0 이상, 상비 재료 제외, 상한 20 | 레시피의 정본이 백엔드 API 로 가고(D-59) 이벤트가 우리 저장소에 쌓이면서 A 의 표는 채워질 길이 없어졌습니다. 이 둘만으로 같은 `UserHistory` 가 나옵니다 | 취향 원본이 DB 로 가면(M-14) 같은 계산을 SQL 로 옮깁니다. 클러스터 관측은 여전히 A 의 배치 몫입니다(G-33) |
...
**2026-09-22: 이 표는 9.31 이전의 상태입니다.** 지금 남은 일의 정본은 `recommend_remaining_work.md` 이고 이 표는 경위로만 남깁니다. 표의 줄 가운데 처리된 것 — 라우터 실연결(M-01) · 서빙 로그 적재(파일로, DB 는 M-05 대기) · 로그 실패 카운터 노출(M-07) · 온보딩 · 이벤트 라우트 실연결(M-15) · 환경변수 목록 확정(`env_variables.md` 1.4.0). 원천이 바뀐 것 — 사용자 이력 피처(M-03)는 A 의 표가 아니라 우리 이벤트 파일과 메모리 사전에서 만듭니다(W-05).
...
### 9.39 2026-09-23 - 병합 확인 · 브랜치 정리 · 팀원용 가이드

| 항목 | 내용 |
|---|---|
| 입력 | 유재현 — "현재 코드까지 모두 머지 되었는지 확인하고 필요하면 PR · 머지까지" → "완료된 우리 브랜치는 삭제해도 좋다. B · C 모든 기능의 가이드 문서를 개발에 참여하지 않은 팀원이 이해하고 발표자료를 만들 수 있게 Notion 으로" |
| 병합 | `feat/recommend-history-features`(4커밋)이 `main` 에 없었습니다 → PR #19 `356be83`(09:04 KST) → 후속 docs PR #20 `96fbd74`. 우리 브랜치 다섯 전부 `main` 에 없는 커밋 0 확인. 데이터 파트의 `develop-data-part`(11커밋) · `data_part_backup` 은 그쪽 몫이라 손대지 않음 |
| 브랜치 정리 | 원격 · 로컬에서 삭제 — `feat/recommend-engine-core` · `feat/recommend-live-serving` · `feat/recommend-monitoring` · `feat/recommend-history-features` · `docs/release-17-merge-record` · `docs/release-19-merge-record`(전부 유재현 작성, `main` 에 병합됨). 로컬의 `feature/Modeling` 은 다른 사람의 옛 브랜치(PR #3 시점)라 둠. 앞으로는 작업마다 새 브랜치 |
| 가이드 | Notion "PantryMate 추천 AI — 파트 B · C 기능 가이드"(비공개 초안, 12절) — 한 장 요약 · 문제와 답 · 전체 그림 · B(입력 · 세 단계 · 취향 · 알레르기 · 이유 · 설계 원칙) · C(여섯 물음 · 네 겹 · 개선 루프) · 다른 파트와의 연결 · 검증 · 발표용 숫자 · 남은 것 · 슬라이드 10장 제안 · 용어 · 문서 지도. 수치는 전부 코드와 검증 기록에서 읽어 넣었습니다(가중치 11개 합 1.0, 손잡이, 경보 9 · 규칙 15 · 패널 33, 검사 621) |
| 환경변수 공유 범위 | 유재현 — "공유해야 할 / 하지 않아야 할 환경변수를 알려 달라, 우리가 전달해야 할 값을 포함해 문서화하고 비공개 문서로 팀원에게" → `docs/env_variables.md` 1.5.0 에 6절(원칙 · 우리가 정해 전달하는 값 · 다른 파트가 만들어 주는 값 · 어디에도 적지 않는 것 · 로컬 최소 설정). 가이드 페이지에 6.1 요약, 전달표는 별도 Notion 비공개 페이지. 비밀 넷의 값은 어디에도 없음. 이어 "실제 전달할 값이 없다, 정해지지 않은 값은 네가 정해서" → 6.6 에 우리가 정한 값(운영 11줄 · 백엔드 5줄 · obs 8줄, RDS `pantrymate-reco-pg16` · `db.t4g.small` 이상, 자원 1/2 CPU · 1/2 GiB, 볼륨 20GiB · 보존 90일, 프로브, 회전 없음, 타임아웃 3.5/5/30초, 시크릿 항목 이름 `pantrymate/...`, 비밀 넷의 생성 규칙)과 4절 각 줄의 제안. 호스트 이름 둘과 포트 8080 은 제안이라 표시. 이어 "BE · 클라우드에 각각 전달할 필수 내용(값)만 축약해서, 받아야 할 것도 각각의 페이지로, 섞지 않게" → Notion 페이지 둘 신설 — "백엔드에 전달"(설정 5줄 · 호출 규칙 · 열어 줄 API 둘 · 알려 줄 것 셋 · 로컬) · "클라우드에 전달"(배포 설정 11줄 · 시크릿 다섯 · RDS · 컨테이너 · 네트워크 · 볼륨 · 받을 것 다섯 · 확인 세 줄). 팀 내부용 전달표는 그대로 둠 |
| 검증 | 코드 변경 없음 |

### 9.38 2026-09-23 - 이력 신호 · 오프라인 평가 · 전달 문서 재구성

| 항목 | 내용 |
|---|---|
| 입력 | 유재현 — "남은 일을 진행해 달라. 전달 문서에서 그들이 조회할 수 없는 절 번호 참조를 빼고, 로컬 테스트 요구사항과 클라우드 배포 요구사항을 분리해 달라." |
| W-05 (D-63) | `engine/history.py` 신설 — 취향 이벤트 + 메모리 사전 → `UserHistory`. 실서빙이 취향 원본을 한 번 읽어 페르소나와 이력에 함께 쓰고, 최근 7일 노출을 메모리 로그에서 찾아 감점 대상으로 넘깁니다. 추적의 조회 단계에 `history_cooked` · `history_liked` · `history_recent`. `service.PersonaService` 에 `profile_for` · `persona_from` 을 갈라 냈습니다. 검사 12 + 1(서빙) + 점검표 M-03 못 교체 |
| W-05 실측 | 실증 덤프에서 조리 20건을 쌓은 사용자 — 두 신호 20/20 켜짐, "지난번 만드신" 사유 6건, 최근 노출 362건 감점, 추천 p50 55 → 90ms · p95 66 → 108ms(조리 20건 x 후보 500건의 IDF 자카드). 콜드 사용자는 그대로 |
| W-07 | `evaluation/offline.py`(순수 함수) + `scripts/eval_recommend_logs.py`. 칸별 반응(탐색은 자기 정규화 IPS) · 순위별 클릭률 · 꺼진 신호 · 엔진 상태 · 날짜별. 어제의 실엔진 로그(추천 335 · 이벤트 208)로 표가 나오는 것을 확인 — 개인화 1.53% · 탐색 1.71%(IPS 1.90%) · 꺼진 가중치 0.26(그때는 이력 전). 검사 7건 |
| 전달 문서 | `recommend_remaining_work.md` 3절을 파트마다 **로컬 테스트 / 클라우드 배포**로 갈랐습니다(B-L1~5 · C-L1~6 · A-L1~3 추가). Notion 세 페이지는 통째로 다시 써서 저장소 절 번호 참조를 없애고 같은 구조로. 백엔드 페이지에는 `docker run` 절차와 "띄운 뒤 확인하실 것" 표, 클라우드 페이지에는 새 환경변수 표를 넣어 자립하게 했습니다 |
| 문서 | 명세 2.5.0(새로고침 때 목록이 달라지는 이유 · 두 신호 켜짐 · 남은 것 둘) · 모니터링 1.5.0 · 점검표 1.11.0(M-03 완료) · 남은 일 1.3.0 · 안건 9.17.0(N-11 닫음) · 진단의 두 신호 안내 문구 |
| 검증 | ruff · format(205) · mypy(79) · pytest 621 passed / 94.3% · 실데이터 종단 35건 — 전부 종료코드 0 |
| 병합 | **PR #19 로 `main` 에 병합**(2026-09-23 09:04 KST, merge commit `356be83`, head `0383a5a`). 유재현의 지시("현재 코드까지 모두 머지 되었는지 확인하고 필요하다면 pr, 머지까지"). 첫 병합 시도는 GitHub 가 병합 가능 여부를 아직 계산하기 전이라 멈췄고 30초 뒤 재시도로 병합. 릴리스 노트는 PR 안에, 병합 정보는 후속 커밋으로 |

### 9.37 2026-09-22 - 마지막 검수와 main 병합 (PR #17)

| 항목 | 내용 |
|---|---|
| 입력 | 유재현 — "마지막 검수를 거치고, 진행해도 좋다면 실 버전으로 머지 진행하자." 이어서 "병합까지 진행해 달라. 병합 뒤에 이어 할 것도." |
| 검수 | ruff · format(200) · mypy 2.3.1(77) · `uv lock --check` · pytest 603 / 94.26% · contract 99 · 시뮬 PASS · 목업 평가 — 전부 종료코드 0. **이미지를 실제로 지어**(2.61GB, 종료코드 0) 이름 붙인 볼륨을 `/app/var` 에 붙이고 대역 백엔드로 띄우니 9.3초에 사전 21,491건 동기화, `/health` `reco-b-linear-v0`, 추천 · 온보딩 · 이벤트 · 로그 조회 · 계약 위반 400 전부 기대대로, uid 10001 이 볼륨에 취향 · 로그 파일을 씀, 메모리 약 600MiB(`docker stats`). 주소 없이 띄우면 `recommend=mock` |
| 검수에서 고친 것 | `catalog_sync_failed` 가 삼킨 예외 규칙(진단 critical · 경보 `RecoSwallowedFailures`)까지 울렸습니다(F-140). 백엔드가 아직 없는 첫 배포에서 "로그 적재 실패" 로 잘못 읽힙니다. 사전 규칙이 따로 보므로 둘에서 뺐습니다(`d3d46e9`). Prometheus 에 다시 읽혀 9개 health ok |
| 병합 | **PR #17** (head `d4dd3ad`, 22 커밋 · 55 파일 · +7,420 / −128) → `main` merge commit **`c8dcaf2`**, 2026-09-22 18:11 KST. 병합 뒤 `main` 의 트리가 브랜치의 트리와 같은 것을 확인. 첫 시도는 도구의 자동 모드 분류기가 "리뷰 없는 병합" 으로 막아 유재현에게 알렸고, 유재현의 두 번째 지시로 다시 실행해 병합했습니다. 릴리스 노트는 PR 안에 넣었고(01의 8절, #15 · #16 분도 채움) 병합 정보는 이 후속 커밋으로 |
| 후속 | 릴리스 노트 2.1 · 2.4 · 2.5~2.7 에 병합 시각 · merge commit · 작업 기록 1절 · 남은 일 1.2.0(W-01 완료). Notion 명세 페이지의 "운영 배포 전" 줄을 병합 사실로 바꿈. 파트별 요청 페이지(백엔드 · 클라우드)에도 병합 사실 |
| 배포 뒤의 동작 | 클라우드가 `BACKEND_BASE_URL` 을 넣기 전까지 운영은 지금과 같은 목업 응답(200). 회귀가 아닙니다. 넣는 순간 실엔진 |
| 다음 | W-05(조리 이력 → 두 신호) → W-07(오프라인 평가). 클라우드 · 백엔드의 답을 기다리는 것은 `recommend_remaining_work.md` 3절 |

### 9.36 2026-09-22 - 남은 일을 우리 것과 요청할 것으로 갈라 문서화

| 항목 | 내용 |
|---|---|
| 입력 | 유재현 — "남은 task 가 뭔가? BE · CL 등 다른 파트에 요청할 내용과 우리가 할 업무를 분리해 문서화하고, 다른 파트 전달분은 파트별 Notion 으로 각각 작성해 달라." |
| 한 일 | `recommend_remaining_work.md` 1.0.0 신설(사람용 사본 포함). 점검표 · 안건 · 모니터링 8절 · 명세 7절 · 인수인계 9 · 13절 · 환경변수 4절에서 남은 것을 모아 우리 일 W-01~W-20, 백엔드 B-01~B-11, 클라우드 C-01~C-12, 데이터 파트 A-01~A-10 으로 번호를 붙였습니다. 이 문서는 목록이고 어긋나면 원천 문서가 이깁니다 |
| 원천의 변화 | M-03(사용자 이력)의 원천이 바뀌었습니다 — A 의 DB 표가 아니라 우리 이벤트 파일 · 취향 이벤트와 메모리 사전(레시피 → 재료)입니다. 다른 파트를 기다리지 않고 할 수 있습니다(W-05) |
| Notion | 파트별 세 페이지(백엔드 · 클라우드 · 데이터 파트)를 비공개 초안으로 작성. 위치는 유재현이 정합니다. 기존 명세 페이지는 그대로 |
| 작업 기록 | 1절의 "브랜치" · "다음 행동" 이 9.24 시점에 멈춰 있던 것을 고쳤습니다. 4.3 표는 경위로만 남기고 정본을 새 문서로 옮겼습니다(W-17 의 절반) |
| 검증 | 코드 변경 없음. `ruff format --check` 만 |
...
| 항목 | 내용 |
|---|---|
| 입력 | 유재현 — "BE 와 CLOUD 에 각각 문서를 전달한 기록이 있을 텐데, 이 문서들도 코드에 맞게 계속 업데이트해 달라." |
| 전달한 문서 | 백엔드 — `backend_api_spec.md`(과 Notion 사본) · `backend_schema_request.md`. 클라우드 — `container_handover.md` · `env_variables.md`. `deploy/SETUP_CLOUD.md` 는 데이터 파트의 문서라 고치지 않았습니다 |
| 방법 | 9.33 에서는 절을 덧붙였을 뿐이라, 이번에는 네 문서를 처음부터 끝까지 지금 코드와 한 줄씩 대조했습니다 |
| 문서가 틀렸던 곳 | 인수인계 5절이 **"쓰기 경로 없음, 읽기 전용 파일시스템 가능"** 이라 적고 있었습니다 — 그대로 배포하면 취향 저장이 전부 500 입니다(F-137). 명세 4.4 가 "받기만 하고 거르지 못한다" · "`PUT /v1/users/{user_id}/pantry` 도 쓸 수 있다" 고 적고 있었습니다 — 실서빙은 그 경로를 읽지 않습니다. 명세 3절이 `category` · `extended_consumption_days` · `updated_at` 을 쓴다고 적었는데 코드는 읽지 않고, 대신 **재료를 이름으로 AI 쪽 사전에 잇는다**는 가장 중요한 사실이 빠져 있었습니다. 4.3 이 온보딩의 알레르기가 추천에 쓰이는 것처럼 읽혔습니다 |
| 코드가 틀렸던 곳 | 대조하다 코드 쪽 결함 셋을 찾아 고쳤습니다. 실서빙이 요청의 `max_missing` 을 읽지 않음(F-134, D-62) · `/health` 가 실서빙에서도 `mock-linear-v0` 을 답함(F-135) · 동기화 스레드가 예상 못 한 예외에 죽으면 사전이 다시는 갱신되지 않는데 에러가 없음(F-136). 검사 셋을 더했습니다 |
| 확인한 것 | "추천은 DB 없이 나간다" 를 문서에 적기 전에 DB 포트를 닫고 실제 HTTP 종단 20건을 돌려 확인했습니다. 경보 규칙 9개는 떠 있는 Prometheus 에 다시 읽혀 전부 health ok |
| 문서 | `backend_api_spec.md` **2.4.0** · `container_handover.md` 1.4.0 · `env_variables.md` 1.3.0(3.5 모니터링 스택 변수 신설) · `backend_schema_request.md` 2.1.0(닫힌 요청임을 밝힘) · `recommend_monitoring.md` 1.3.0. **Notion 사본도 2.4.0 으로 맞췄습니다**(맨 위에 "달라진 것 여섯" 과 "이 판의 코드는 브랜치에 있고 운영 배포 전" 이라는 줄을 뒀습니다 — 배포되면 그 줄을 지웁니다) |
| 검증 | ruff · format(198) · mypy 2.3.1(77 files) · pytest 601 passed / 94.26%(`PYTHONUTF8=1`) · contract 99 · 실제 HTTP 종단 20건(DB 있음 · 없음 둘 다) — 전부 종료코드 0 |
| 사고 | 따옴표 없는 heredoc 에 백틱이 든 글을 넣어 셸이 그 안의 경로를 명령으로 실행하려 했습니다(`deploy/docker-compose.yml` 등). 전부 "command not found" 로 끝났고 작업본은 `git status` 로 확인해 변한 것이 없었습니다. 그 뒤로 글이 든 스크립트는 파일로 만들어 실행합니다 |
| 밖에 알릴 것 | N-21 그대로 — 클라우드에 인수인계 1.4.0 과 환경변수 1.3.0, 백엔드에 명세 2.4.0. 데이터 파트 — `deploy/SETUP_CLOUD.md` 의 "①② 까지만 해도 서비스는 뜨지만 추천할 레시피가 없다" 는 설명이 옛것이 됐습니다(추천의 레시피는 이제 백엔드에서 옵니다) |
...
| 항목 | 내용 |
|---|---|
| 입력 | 유재현 — "댓글은 BE 팀장이 달아 준 것이다. 가능한 부분은 모두 의견을 반영해서 코드를 수정해 보자." 댓글 본문을 글로 전달(4.3 · 4.4 둘 · 7.3 · 7.4 · 8절 둘) |
| 앞 세션의 결과 | 9.31 의 커밋은 **PR #15 로 main 에 병합됐습니다**(`750afdb`, merge commit, 64 커밋). 병합 뒤 main 의 트리와 브랜치의 트리가 같은 것을 확인했고 브랜치를 main 으로 맞췄습니다 |
| 유형 코드 (D-58) | 대소문자는 이미 받고 있었습니다. `ETC` → `asian_other` 만 더했습니다. 검사 7건 |
| 요청 계약 (D-57) | `RecommendRequest` 에 `pantry` · `allergies` 필수. 냉장고 항목에 수량 · 단위 없음. 목업은 받은 값을 추적(`pantry_received` · `allergy_labels` · `allergy_labels_unknown`)과 로그(`pantry_snapshot` · `pantry_detail`)에 남깁니다. 검사 5건 |
| 판본 묶기 (D-56) | `engine/dish.py` 신설 · `policy.max_per_dish` · `service.rank_candidates` 에 연결 · 추적 `dropped.same_dish`. 기준을 실데이터로 골랐습니다(F-126~F-128). 검사 33건 |
| 빠진 재료 행 | 코드 변경 없음. 백엔드가 재가공을 확답하지 못해 제목 방어선이 계속 유일한 안전판입니다 |
| 고친 호출부 | 필수 둘 때문에 기존 호출이 전부 400 이 됩니다 — `test_contract.py` · `test_writer.py` · `test_cuisine.py` · `test_main.py` · `scripts/reco/api/capture.py` · `render.py` · `doc_check.py` · `scripts/sim/scenario_run.py` 를 새 계약에 맞췄습니다 |
| 명세 · Notion | `docs/backend_api_spec.md` **2.2.0** — 4.3 유형 코드 · 4.4 필수 둘과 수량 제거 · 7.3 · 7.4 회신 · 8절 회신 표 · 9절. Notion 사본도 같은 내용으로 |
| 검증 | ruff · format(181) · pytest 491 passed / 93.18% · contract 99 · eval · 시뮬 PASS · 실데이터 종단 87건 — 전부 종료코드 0. mypy 는 우회로로 종료코드 1, 앞과 같은 2건 |
| 남의 파일 | `enums.py` · `schema.py` · `engine/mock.py` · `test_contract.py` · `scripts/reco/api/*` · `doc_check.py`(A) |
| 밖에 알릴 것 | **대시보드(C)의 `/v1/recommend` 호출이 400 이 됩니다.** `"pantry": [], "allergies": []` 를 더해야 합니다 |

### 9.31 2026-09-21 - 백엔드 회신 반영과 main 병합 준비

| 항목 | 내용 |
|---|---|
| 입력 | 유재현 — "지금까지 개발된 내용을 main 과 비교해 머지를 진행해 달라." 이어서 "그 전에 수정할 부분이 생겼다. 백엔드가 명세 8절과 3.1 에 답글을 달았다. 의견을 반영해 코드를 수정하고 main 에 머지하자." |
| main 과 비교 | 브랜치가 `origin/main` 보다 62 커밋 앞서고 뒤처진 것은 0 입니다(병합 커밋 7 포함, 유재현 19 · QDevJw 36). 충돌 없음. 100 파일 +9,870 / -3,108. 비밀값 패턴 0건, `.env` · 덤프 · 가중치 없음 |
| 백엔드 회신 | ① 검증 실패 응답에 이유를 실어 달라 ② 스크랩 수는 즉시 제공, 조회수는 레시피 테이블에 컬럼 추가 예정 ③ 3.1 은 공통 응답 래퍼 없이 명세 구조 그대로. 8절의 나머지 다섯 행은 댓글이 접혀 있어 읽지 못했습니다. Notion API 로는 댓글이 조회되지 않았습니다(빈 응답) |
| 고친 것 (F-125 · D-55) | `schema.ValidationErrorOut` · `FieldErrorOut` · `validation_error_body()` 추가, `main.handle_missing_field` 가 영수증 경로 밖에서 그 본문을 싣습니다. 검사 4건(`tests/unit/test_main.py`). A 의 문서 생성기(`scripts/reco/api/render.py` · `capture.py`)에서 "본문 없음" 문구를 고쳤습니다 |
| 코드를 안 고친 것 | ② 와 ③ 은 아직 없는 동기화 코드가 읽을 모양입니다. 명세에만 반영했습니다 — `popularity.scrap_count` 를 합의 필드로, `view_count` · `order_count` 는 선택으로, 3절에 "래퍼 없음" |
| 명세 | `docs/backend_api_spec.md` **2.1.0** — 6.1 검증 실패의 본문(실 서버 응답 예시) · 8절에 백엔드 회신 열 · 9절 구현 상태 |
| 남의 파일 | `src/main.py`(영수증 파트) · `schema.py` · `render.py` · `capture.py`(A). 영수증 경로의 동작은 그대로이고 그 검사도 그대로 통과합니다 |
| 검증 | ruff · format(179) · pytest 446 passed / 92.79% · contract 99 — 전부 종료코드 0. mypy 는 우회로(1.11.0)로 종료코드 1, 2건 모두 main 에 이미 있는 줄(검증 21.15) |
| 병합 | PR 생성과 병합은 이 기록 커밋 뒤에 진행합니다. 결과는 다음 세션 기록에 적습니다. Squash 가 아니라 merge commit 을 씁니다 — 이 브랜치가 A 의 36 커밋을 들고 있어 Squash 하면 A 브랜치가 나중에 같은 변경으로 다시 충돌합니다 |
...
| 항목 | 내용 |
|---|---|
| 입력 | 유재현 — "바로잡은 것에서 결함이 그대로였다는 건 문제가 해결되지 않았다는 이야기 같다. 다시 수정을 시도해 달라. 알레르기는 백엔드 DB 에 있고 `[우유, 땅콩]` 식으로 API 로 받을 예정이다. 지금은 데이터 없이 또는 임의 데이터로 테스트해 달라" |
| 제 말의 잘못 | "결함은 그대로" 는 **진단이 교정 뒤에도 재현된다**는 뜻이었는데 안 고쳐졌다고 읽히게 썼습니다. F-111 의 증상(점수 전원 1.000)은 N-19 로 고쳐져 있었습니다. 다만 다시 재 보니 **남은 결함이 실제로 있었습니다** |
| 남은 결함 (F-119) | 개인화 16칸의 경계 점수에 9~14건이 동점이고 그중 누가 들어갈지를 `recipe_id` 가 정했습니다. 같은 냉장고의 40명이 전원 같은 16건을 받았습니다. `score.tie_break` 로 고쳤습니다(D-52). 수정 뒤 28종 · 25가지 목록 |
| 알레르기 | `engine/allergy.py` 신설(D-53). 한글 라벨 → 시드의 군 전체 + 이름 규칙. 모르는 라벨은 돌려줍니다. 임의 데이터 9벌에서 새는 것 0건 |
| 돌리다 찾은 것 | ① `통깨` 가 시드에 안 닿아 참깨 알레르기가 샜습니다 — 시험대에만 있던 별칭이 틀렸고, 통깨는 레시피의 32.8% 입니다(F-120) ② **새우 알레르기 사용자의 1위가 「고추장 건새우볶음」** — 그 레시피의 재료 행에 건새우가 없습니다(F-121). 제목을 함께 보는 `allergy.blocks` 로 막았습니다(D-54) ③ 제목 검사의 `잣` 이 「감잣국」 에 걸렸습니다(F-123) ④ 제가 검증 21.8 에 "명세에 이미 적혀 있다" 고 쓴 문장이 사실이 아니었습니다(F-122) |
| 코드 | `engine/allergy.py`(신규) · `engine/score.py`(`tie_break` · `order_key`) · `engine/rerank.py` · 검사 30건(`test_allergy.py` 26 · `test_score.py` 4) · 픽스처 `backend_ingredients.json`(재료 이름 사본 148종) |
| 검증 | ruff · format(173) · pytest 400 passed / 91.51% · contract 98 · eval · 시뮬 PASS 전부 종료코드 0. mypy 는 우회로로 68 files, 새 코드 보고 0건. 실데이터 종단 87건 통과 |
| 문서 | API 명세 1.1.0(알레르기 라벨 표 · `is_staple` · `allergens` 배열 · 6.3 재료 행 누락) · 검증 21.10~21.12 · 계획서 2.4.0 · 안건 G-34 확장 |
| 넘긴 것 | 백엔드 — 라벨 목록 공유 · 빠진 재료 행 복구 · `allergens` 배열 · `is_staple`(G-34). **못 막는 것 하나**: 간장의 밀처럼 가공품 안의 다른 알레르기(시드가 재료당 군을 하나만 답니다) |

### 9.28 2026-09-21 - N-19 반영과 타입 검사 우회로

| 항목 | 내용 |
|---|---|
| 입력 | 유재현 — "N-19 는 네 제안대로 진행해 줘. 그리고 mypy 를 돌리지 못하는 이 PC 의 제어 정책을 어떻게 처리할지도 알려 줘. 권한을 부여할 수 있다면 부여할게" |
| N-19 | 선택지 (d) 반영. `f_coverage` 0.24 → 0.14 · `f_pantry_use` 0 → 0.10. **어디서 뺄지를 다시 측정했습니다** — 인기도에서 빼는 쪽과 충족률에서 빼는 쪽의 결과가 같았고, 인기도를 0 으로 내리면 "데이터가 오면 코드를 고치지 않아도 켜진다" 는 규칙을 어기므로 충족률에서 가져왔습니다. 재료 매칭 군 합 0.44 와 `ACTIVE_WEIGHT_TODAY` 0.94 는 불변입니다 (검증 21.6) |
| 제어 정책 | **Smart App Control** 입니다(`VerifiedAndReputablePolicyState = 1`). 코드 무결성 로그가 막은 파일을 이름으로 적고 있습니다 — `mypy.exe` · `pytest.exe` · `librt/internal.pyd` · `coverage/tracer.pyd`. 경로와 무관하고(OneDrive 밖으로 복사해도 같음) 다운로드 표식 때문도 아니며 예외 목록이 없는 정책입니다. **끄면 윈도우를 다시 설치하기 전까지 다시 켤 수 없습니다** |
| 우회로 | 컴파일 확장이 없는 `mypy==1.11.0` 을 별도 환경에 `--no-binary` 로 설치해 돌립니다. 67개 파일 6분 19초, 보고 2건은 옛 판의 추론 차이라 고치지 않았습니다 (검증 21.7) |
| 넘긴 것 | N-20 — 타입 검사를 어디서 돌릴지(도커 · 옛 판 · CI · 정책 해제). B 제안은 도커 기본, 급할 때 옛 판 |
