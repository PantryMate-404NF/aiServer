...
**버전**: 1.31.0 · **최종 수정**: 2026-09-23 · **작성자**: 유재현
...
| 브랜치 | `feat/recommend-live-serving` 은 **PR #17 로 `main` 에 병합됐습니다**(2026-09-22 18:11 KST, merge commit `c8dcaf2`, head `d4dd3ad`). 병합이 곧 배포이므로 그 시점부터 `main` 의 코드가 운영입니다 — 클라우드가 `BACKEND_BASE_URL` 을 넣기 전까지는 목업으로 답합니다. 앞선 브랜치 `feat/recommend-engine-core` 는 PR #15(`750afdb`) · #16(`90126c2`) |
...
| 병합 | 하지 않았습니다. 브랜치 `feat/recommend-history-features` 에 push. 병합은 배포이므로 지시를 기다립니다 |
