...
| 환경변수 공유 범위 | 유재현 — "공유해야 할 / 하지 않아야 할 환경변수를 알려 달라, 우리가 전달해야 할 값을 포함해 문서화하고 비공개 문서로 팀원에게" → `docs/env_variables.md` 1.5.0 에 6절(원칙 · 우리가 정해 전달하는 값 · 다른 파트가 만들어 주는 값 · 어디에도 적지 않는 것 · 로컬 최소 설정). 가이드 페이지에 6.1 요약, 전달표는 별도 Notion 비공개 페이지. 비밀 넷의 값은 어디에도 없음 |
| 검증 | 코드 변경 없음 |
