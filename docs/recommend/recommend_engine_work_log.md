...
| 다음 행동 | **9.32 의 커밋을 main 에 올릴지 유재현 결정(PR #15 뒤의 것이라 브랜치에만 있습니다)** → 백엔드의 엔드포인트 주소가 오면 레시피 · 재료 동기화 구현 → 라우터 실연결(M-01·M-02): `req.pantry` 를 `build_context(own_pantry_ids=...)` 로, `req.allergies` 를 `allergy.resolve()` · `blocks()` 로 잇습니다. 동기화 때 `dish.dish_name` 을 미리 돌려 두면 첫 요청의 101ms 가 사라집니다. 알릴 것 — A: 9.30 의 알레르기 표 단일화, 9.31 의 400 본문, **9.32 의 요청 필수 둘(`test_contract.py` · `capture.py` · `doc_check.py` · `render.py` 를 고쳤습니다)**. C(대시보드): **`/v1/recommend` 에 `pantry` · `allergies` 가 없으면 400 입니다.** 영수증 파트: `main.handle_missing_field` 가 영수증 경로 밖에서 본문을 싣습니다 |
