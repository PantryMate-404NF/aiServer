...
| `reco_serving_live` · `reco_catalog_ready` | | 실서빙인가(1) 목업인가(0) · 레시피 사전을 받았는가 |
| `reco_catalog_recipes` · `reco_catalog_age_seconds` | | 사전의 레시피 수 · 마지막 동기화로부터 지난 시간. 누적이 아니라 지금 값입니다 |
...
| 심각도 | 규칙 | 기준 | 다음에 할 일 |
|---|---|---|---|
| critical | 레시피 사전이 없음 | 실서빙인데 사전을 못 받음 | 추천이 전부 503 입니다. 백엔드가 인기순으로 대신해 화면은 멀쩡해 보입니다. 주소 · 경로 · 방화벽 · 키를 봅니다 |
| warning | 레시피 사전이 오래됨 | 마지막 동기화로부터 이틀 초과 | 동기화가 계속 실패하고 있습니다. 어제의 사전으로 서빙 중입니다 |
| critical | 막지 못한 알레르기 라벨 | 1건 이상 | 서버 로그에서 라벨을 찾아 `enums.ALLERGEN_LABELS` 나 `engine/allergy.py` 의 동의어에 더합니다 |
| critical | 삼킨 예외 | 내부 카운터의 `fail` · `error` 1건 이상(사전 동기화의 실패는 제외 — 아래 사전 규칙이 봅니다) | 로그 적재나 취향 저장이 실패했는데 200 이 나간 것입니다 |
...
| `http://localhost:9090/alerts` | 경보 규칙 9개의 상태 |
...
**백엔드 주소가 없는 서버의 수치는 목업의 것입니다.** 실서빙은 2026-09-22 에 연결됐고 `BACKEND_BASE_URL` 이 설정된 서버에서만 돕니다. 관측은 둘을 `engine` 라벨로 가르고 관리자 페이지 맨 위에 그 사실을 띄웁니다. 목업은 신호 값을 난수로 채우므로 "꺼진 신호" 규칙은 실서빙에서만 의미를 갖습니다. 실서빙에서 두 신호(선호 재료 · 재료 궁합, 가중치 합 0.21)는 **조리 · 저장 이벤트가 쌓인 사용자에게만** 켜집니다(2026-09-23, M-03). 이벤트가 오기 전에는 꺼진 것으로 나오고 그것이 맞습니다. 백엔드가 별점을 주기 전에는 품질 신호도, 냉장고 항목에 날짜가 실려 오기 전에는 임박 신호도 꺼져 있습니다. 실제 목록과 이유는 관리자 페이지가 보여 줍니다.

**관리자 페이지는 지금 상태만 봅니다.** 서버가 뜬 뒤의 누적이라 재시작하면 0 입니다. 지난주와 비교하는 일은 Grafana 에서 합니다.

**실엔진 트래픽으로 끝까지 돌려 본 결과(2026-09-22, 9.35).** 대역 백엔드(실증 덤프 21,491건) 위의 실서빙에 추천 350건 · 온보딩 20건 · 이벤트 358건을 보내고 Prometheus 가 긁게 둔 뒤 — 대시보드의 식 40개 전부 값 있음 · 경보 9개 전부 health ok 이고 셋이 의도대로 firing(모르는 라벨 · 후보 부족 · 계약 위반, 전부 일부러 섞은 것) · 요약이 실엔진 `engine=real` 로 할 일 일곱을 냄 · Grafana 가 대시보드(패널 33 + 행 6)를 읽고 그것을 거친 질의가 값을 돌려줌 · `/metrics` 와 요약은 키 없이 401. 그 과정에서 진단 하나를 고쳤습니다 — 추천 계약 위반의 근거에 온보딩의 400 이 섞여 있었습니다. 엔진 p95 는 184ms 로 목표 58ms 를 넘습니다(정보 등급). 목표는 목업 시절의 값이라 실데이터 분포로 다시 정할 일입니다(8절의 3).

**패널의 모양은 사람이 눈으로 보지 않았습니다.** 2026-09-22 에 실제로 띄워 API 로 확인한 것은 이렇습니다 — 수집 대상 UP(시크릿 파일의 Bearer 키로 `/metrics` 통과) · 경보 규칙 7개 전부 health ok 이고 `RecoUnknownAllergyLabel` 이 의도대로 firing · 대시보드의 식 36개를 Prometheus 에 전부 물어 문법 오류 0 · Grafana 가 데이터소스 둘과 대시보드(패널 29)를 읽었고 Grafana 를 거친 질의가 값을 돌려줌. 같은 날 레시피 사전 규칙 둘과 패널 넷을 더한 뒤(지금 규칙 9개 · 패널 33) Prometheus 에 규칙을 다시 읽혀 9개 전부 health ok 인 것을 확인했습니다. 더한 패널 넷은 Grafana 에 다시 올려 보지 않았습니다. 그 과정에서 하나를 고쳤습니다. 비율 통계 넷이 분자의 시계열이 아직 없을 때 0 이 아니라 "값 없음" 으로 나와 `or vector(0)` 를 붙였습니다. 색 · 단위 · 범례가 보기 좋은지는 열어서 봐야 합니다.

**워커가 하나일 때만 맞습니다.** 등록부가 프로세스마다 따로라, uvicorn 워커를 늘리면 수집 때마다 다른 워커가 답해 수치가 널뜁니다. 늘릴 때는 `prometheus_client` 의 다중 프로세스 모드로 바꿔야 합니다.

**오프라인 평가는 `scripts/eval_recommend_logs.py` 가 냅니다**(2026-09-23). `RECO_LOG_DIR` 의 두 JSONL(`recommendations-*.jsonl` · `events-*.jsonl`)을 `request_id` 로 이어 칸별 반응(클릭 · 저장 · 조리, 탐색 칸은 자기 정규화 IPS 로 보정), 순위별 클릭률, 꺼진 신호와 그 가중치, 엔진 상태(후보 부족 · 사다리 · 판본 탈락 · 지연 · 모르는 라벨), 날짜별 추이를 표로 냅니다(`--json` 으로 저장). 계산은 `evaluation/offline.py` 에 있습니다. nDCG 같은 순위 지표와 가중치 재유도는 아직 없습니다 — 라벨이 쌓인 뒤의 일입니다.
...
| 2 | 배포된 서버에서 "꺼진 신호" 와 칸별 클릭률을 기준선으로 기록 | 백엔드의 두 API 와 `BACKEND_BASE_URL` |
| 3 | 임계값을 실제 분포로 다시 정함 | 운영 트래픽 2주 |
| 4 | 가중치 재유도의 입력으로 칸별 · 순위별 반응을 내보냄 | 클릭 이벤트 누적. 읽는 스크립트는 있습니다(`scripts/eval_recommend_logs.py --json`) |
