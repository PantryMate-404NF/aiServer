...
**백엔드 주소가 없는 서버의 수치는 목업의 것입니다.** 실서빙은 2026-09-22 에 연결됐고 `BACKEND_BASE_URL` 이 설정된 서버에서만 돕니다. 관측은 둘을 `engine` 라벨로 가르고 관리자 페이지 맨 위에 그 사실을 띄웁니다. 목업은 신호 값을 난수로 채우므로 "꺼진 신호" 규칙은 실서빙에서만 의미를 갖습니다. 실서빙에서는 적어도 두 신호(선호 재료 · 재료 궁합, 가중치 합 0.21)가 꺼진 것으로 나옵니다 — 사용자의 조리 이력을 엔진에 잇는 일(DB 전환 M-03)이 남아 있습니다. 백엔드가 별점을 주기 전에는 품질 신호도, 냉장고 항목에 날짜가 실려 오기 전에는 임박 신호도 꺼져 있습니다. 실제 목록과 이유는 관리자 페이지가 보여 줍니다.
...
**오프라인 평가 도구는 없습니다. 재료는 쌓입니다.** nDCG 같은 순위 지표와 IPS 보정은 라벨이 쌓인 뒤의 일입니다. 그 재료 — 추천마다 칸 · 순위 · 노출확률 · 신호 17종, 이벤트마다 `request_id` · 순위 · 종류 — 는 실서빙이 `RECO_LOG_DIR` 의 두 JSONL 파일(`recommendations-*.jsonl` · `events-*.jsonl`)에 남깁니다(2026-09-22). 두 파일을 `request_id` 로 이어 칸별 클릭률과 순위별 반응이 계산되는 것을 실엔진 트래픽으로 확인했습니다(9.35). 읽어서 계산하는 스크립트는 아직 없습니다.
...
| 4 | 가중치 재유도의 입력으로 칸별 · 순위별 반응을 내보냄 | 클릭 이벤트 누적. 원본은 `RECO_LOG_DIR` 의 두 JSONL — 읽는 스크립트부터 만듭니다 |
