...
| F-140 | 사전 동기화 실패가 삼킨 예외 경보까지 울림 | `catalog_sync_failed` 에 "fail" 이 들어 있어 `_rule_internal_failures` 와 경보 `RecoSwallowedFailures`(`key=~".*(fail|error).*"`)에 걸렸습니다. 그 규칙의 안내는 "로그 적재나 취향 저장이 실패했는데 200" 이라, 백엔드가 아직 안 떠 있는 첫 배포에서 잘못 읽힙니다. 9.35 의 트래픽에서는 동기화가 성공해 드러나지 않았고 병합 직전 검수에서 코드를 읽다 찾았습니다 | 진단과 경보 둘에서 `catalog_` 키를 뺍니다(사전 규칙이 봅니다). `test_a_failed_catalog_sync_is_the_catalog_rules_business_not_a_swallowed_failure`. 떠 있는 Prometheus 에 다시 읽혀 9개 health ok |
