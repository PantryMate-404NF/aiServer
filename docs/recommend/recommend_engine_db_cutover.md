...
| M-03 | 사용자 이력 적재 | `UserHistory` 가 항상 비어 있어 `f_ing_pref`(0.11)·`f_cooccur`(0.10)가 **전건 None** 입니다. 클러스터 관측도 비어 Thompson 이 균등 사전분포로만 돕니다 (F-15, F-28) | `user_ingredient_pref`·`event_log`·`user_cluster_stat` 에서 읽는 저장소 함수를 만들고(조리 레시피의 재료 집합과 제목 `cooked_titles` 포함 — 없으면 "지난번 만드신 X 와 비슷해요" 사유가 나오지 않습니다) `build_context` 에 넣습니다 | 12 페르소나 재실행에서 두 피처가 None 이 아니고, 죽은 가중치 합이 0.26 → 0.05 로 줄어듦 | 완료(09-23) — **원천이 바뀌었습니다.** A 의 표 대신 우리 취향 이벤트와 메모리 사전에서 `engine/history.py` 가 `UserHistory` 를 만듭니다(최근 14일 조리 → 재료 집합 · 제목, 조리 · 저장 · 클릭 · 별점의 재료를 종류 무게 x 시간 감쇠로 세어 1.0 이상만 선호, 상비 재료 제외, 최근 7일 노출은 메모리 로그에서). 실데이터에서 조리 20건을 쌓은 사용자의 두 신호가 20/20 켜지고 사유 6건, 추천 p50 55 → 90ms. 클러스터 관측(`cluster_seen` · `cluster_hits`)은 여전히 비어 있습니다(G-33). `test_a_cook_switches_on_the_two_history_signals_for_the_next_recommendation` |
