...
**버전**: 2.4.0 · **최종 수정**: 2026-09-21 · **작성자**: 유재현
...
3. **알레르기는 ① 에서 완전 제외.** 감점 대상이 아니라 조건으로 후보에서 뺍니다. 제외는 ① 에서만 하고 ②③ 은 후보를 빼지 않습니다 — 그래야 "왜 빠졌는가" 를 한 곳에서 찾습니다. 백엔드는 알레르기를 한글 라벨(`["우유", "땅콩"]`)로 보내며 `engine/allergy.py` 가 그것을 막을 재료로 풉니다. 라벨은 시드의 군 전체를 막고(땅콩 → 견과 전부), 모르는 라벨은 버리지 않고 돌려주며, 재료 행이 빠진 레시피를 잡으려고 **제목도 함께 봅니다**(`allergy.blocks`).
...
|---|---|---|---|---|
| 재료 | `f_coverage` | ① 이 준 coverage = 1 − 부족 수 / 필수 수 | 없음 | 0.14 |
| 재료 | `f_missing` | 1 − 부족 수 / (max_missing + 1) | 없음 | 0.05 |
| 재료 | `f_expiring` | 임박 재료 가운데 필수 재료에 쓰이는 비율 | 임박 재료가 없음 · 레시피 재료를 모름 | 0.15 |
| 재료 | `f_pantry_use` | 보유 재료 가운데 레시피에 쓰이는 비율 | 보유 재료가 없음 · 레시피 재료를 모름 | 0.10 |
...
곱한 계수는 소수 여섯 자리로 반올림한 값을 점수에 곱하고 그대로 `penalty` 로 남겨, 로그의 두 값만으로 점수가 정확히 재현됩니다. 후보는 점수 내림차순이고, 같은 점수는 `user_id` 와 `recipe_id` 로 만든 안정 해시 순입니다(`score.tie_break`). 번호 순으로 두면 낮은 번호가 모든 사용자에게 언제나 이겨 뒷번호 레시피가 영영 노출되지 않습니다 — 실데이터에서 개인화 경계에 9~14건이 동점으로 걸렸습니다. 난수는 쓰지 않습니다. 개인화 칸은 같은 요청에 같은 답을 내야 하고 로그만으로 재현돼야 하기 때문입니다. ① 이 준 후보 가운데 피처 행이 없는 것과 같은 레시피가 두 번 온 것은 점수를 매기지 않고 세어 ranking 단계의 `filters`(`recipe_feature_missing`, `candidate_duplicate`)에 남깁니다 — 피처가 없는 후보는 갖춘 재료 하나만으로 만점이 되어 1위로 나가기 때문입니다.
...
scripts/generate_mock_fixtures.py   가상 사용자 12명 · 레시피 120 · 재료 60 · 알레르기 그룹 10 (tests/fixtures/recommend/)
...
| 가상 인벤토리 | 12명(취향·냉장고·알레르기·조리 시간 상한이 다름), 레시피 120, 재료 60, 알레르기 그룹 10(시드에서 읽음). 생성기는 해시로 결정론을 확보하고 페르소나마다 척도에 가장 가까운 제시 음식 4개를 고른 것으로 둡니다(척도만 있는 사람 1, 취향 없는 사람 1 포함) |
...
`f_taste` 0.16 · `f_expiring` 0.15 · `f_coverage` 0.14 · `f_ing_pref` 0.11 · `f_cooccur` 0.10 · `f_popularity` 0.10 · `f_pantry_use` 0.10 · `f_missing` 0.05 · `f_cuisine` 0.04 · `f_time_fit` 0.03 · `f_season` 0.02, 나머지 6종 0. 합 1.00. 2026-09-21 에 재료 매칭 몫 0.44 안에서 `f_coverage` 0.24 → 0.14 · `f_pantry_use` 0 → 0.10 으로 옮겼습니다 — 실데이터에서 충족률이 포화해 순위를 못 만들기 때문입니다(N-19, 검증 21.6). **임시값이며** 인기도 신호가 들어오면 다시 잽니다. 실제 사용자 데이터에서 쌍대비교(Bradley-Terry) 학습으로 다시 정하고, 정한 값은 `scoring_config` 에 `config_hash` 로 등록합니다.
