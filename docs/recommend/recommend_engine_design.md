...
**버전**: 2.2.0 · **최종 수정**: 2026-09-15 · **작성자**: 유재현
...
| 레시피 쪽 음식 유형 | `recipe.cuisine_family` 가 46,353건 전수 비어 있어 실 DB 에서는 `f_cuisine` 과 유형 슬롯이 둘 다 돌지 않습니다. 목업·시뮬 카탈로그에서만 동작을 확인했습니다 | 태그 → 유형 매핑을 파트 A 가 언제 넣을지. 그 전까지 온보딩 문항은 저장만 되고 목록에 닿지 않습니다 |
