...
**버전**: 2.2.1 · **최종 수정**: 2026-09-18 · **작성자**: 유재현
...
| `POST /v1/onboarding/{user_id}` | 온보딩 6문항 저장 | `OnboardingIn` | `OnboardingOut` |
...
**`OnboardingIn`**: `picks`(제시 음식 20개 가운데 고른 것의 인덱스 — 레시피 ID 가 아닙니다, 1~20개), `scales`(3축 [매움, 짠맛, 단맛] 각 0~4), `preferred_cuisines`(좋아하는 음식 유형, 최대 5), `allergy_groups`, `allergy_ingredient_ids`, `avoid_ingredient_ids`(최대 3), `household_size`. 고른 개수는 서버가 강제하지 않고(프론트의 일) 3개 미만이면 셉니다. **`OnboardingOut`**: `user_id`, `taste_vec`(6축), `n_blocked_ingredients`, `preferred_cuisines`(저장된 코드).

**음식 유형(`preferred_cuisines`)** 은 `korean` · `chinese` · `japanese` · `western` · `asian_other` 다섯 가지이고 한글 라벨(한식 · 중식 · 일식 · 양식 · 아시안)로 보내도 코드로 바꿔 저장합니다. 값의 정본은 `seeds/cuisine_taxonomy.yaml` 의 `family` 축이며 레시피 쪽 `recipe.cuisine_family` 와 같은 축입니다 — 라벨로 저장하면 두 축이 영영 안 만납니다. **모르는 값은 가까운 유형으로 바꾸지 않고 거부합니다.** 바꿔 넣으면 사용자가 고르지 않은 유형의 음식이 목록에 올라오는데 응답은 200 이라 아무도 모릅니다. 저장 위치는 `user_preference.pref_cuisines` 이고, 실 DB 전까지는 취향 원본 JSON(`TasteProfile.cuisines`)입니다.
...
요리군은 2.3 의 음식 유형 다섯 가지와 같은 축입니다(09-15). 그 전에는 다섯째가 "기타" 한 묶음이라 태국·베트남과 멕시칸·아메리칸이 같은 이름이었고, 어느 쪽으로도 사용자의 선택과 만나지 못했습니다. 제시 목록 자체는 바꾸지 않고 이름만 가른 것이라 각 음식의 6축 값은 그대로입니다.
...
**음식 유형은 이 6축에 섞지 않습니다.** 고른 유형의 평균 맛을 취향에 더하면 "한식을 좋아함"이 "짜고 매운 것을 좋아함"으로 번역되어, 문항 하나가 맛 취향을 통째로 움직입니다. 유형은 다른 축으로 따로 가며 3.4 의 유형 슬롯과 `f_cuisine` 이 씁니다.
...
가중치 0 인 피처는 재기만 하고 순위에 쓰지 않습니다. 가중치의 합은 1.00 이고, 그 가운데 이력이 있어야 재는 `f_ing_pref`·`f_cooccur`(0.21)와 데이터가 없거나 일부만 있는 `f_cuisine`(09-17 부터 레시피 61.7% 배정)·`f_season`(0.06)은 지금 Mock 에서 대부분 모름이라 순위에 관여하지 않습니다. 데이터가 오면 코드를 고치지 않아도 켜집니다. 가중치는 실제 사용자 데이터로 쌍대비교 학습을 해 다시 정합니다. 디버거가 요청에 실어 보내는 가중치 덮어쓰기는 모르는 키·음수·합 0 을 거부합니다 — 오타 하나가 조용히 한 피처의 몫을 없애기 때문입니다.
...
**음식 유형 슬롯.** 온보딩에서 고른 유형이 목록에 한 건도 없을 때만, 몇 칸을 그 유형 몫으로 뗍니다.

```text
칸 수 n      = min(round(top_k × 0.1), 2)   → Top-20 에서 두 칸. 행동이 쌓인 사용자(behavior)는 0
대상 유형     = 고른 유형 중 개인화·탐색 목록에 한 건도 없는 것 (있으면 뗄 이유가 없다)
후보         = 개인화·탐색에 들지 못한 잔여 후보 가운데 점수가 중위수 이상인 것
고르는 순서   = 유형 안에서 맛 페르소나에 가장 가까운 것(f_taste), 같으면 점수 순. 유형마다 한 칸까지
뺄 자리       = 개인화 꼬리부터. 단 고른 유형의 마지막 한 건은 건너뜀 (그것을 빼면 넣은 만큼 지운 셈)
자리         = 목록 중반부터 고르게. 탐색이 가져간 자리는 비켜 감
노출 확률     = 1.0 (결정적 슬롯). 탐색과 다른 칸이라 `is_cuisine_slot` 으로 따로 기록
```

한 유형에서 두 개째를 가져오지 않는 것과, 뺄 자리에서 다른 유형의 마지막 한 건을 건너뛰는 것은 같은 이유입니다 —
둘 다 "고른 유형이 목록에 보이는 것" 에 보태는 것 없이 다른 유형을 목록에서 지웁니다. 실제로 중식이 20위,
일식이 20위 밖에만 있는 후보에서 일식 칸을 떼자 중식이 통째로 사라지는 것을 확인했습니다(F-91).

가중치를 올리지 않고 자리를 떼는 이유는 순위를 건드리면 유형이 사실상 주 순위가 되기 때문입니다.
Σw 의 0.44 가 재료 매칭이고 유형이 뒤집을 수 있는 것은 `f_cuisine` 0.04 뿐이라, 고른 유형의 레시피가
냉장고 재료와 덜 겹치면 Top-K 에 한 건도 안 들어옵니다. 그렇다고 가중치를 올리면 고른 유형 하나가
20칸을 물들입니다. 자리를 떼면 18칸은 그대로이고 두 칸이 유형 몫이며, 그 칸의 이유 문구도 유형으로
적힙니다("즐겨 드시는 중식이고, 많이 만드는 레시피예요").
...
동결 키는 아니지만 요청마다 덧붙는 값도 있습니다. 음식 유형을 고른 사용자에게는 `n_cuisine`(유형 칸 수) · `cuisine_slot_ratio` · `preferred_cuisines` 가 실리고, 고른 유형이 끝내 목록에 없으면 `cuisine_unmet` 이 함께 실립니다. 칸 수만으로는 "이미 목록에 있어서 0"과 "후보에 그 유형이 없어서 0"이 구분되지 않습니다.
...
└── engine/
    ...
    ├── cuisine.py      [B] 음식 유형 슬롯 — 칸 수 · 빠진 유형 · 맛 기준 선택 · 자리
    ├── rerank.py       [B] MMR · 혼합 탐색 · 유형 슬롯 · 균등 폴백 · 위치 · 이유 → RankedItem
...
| ③ 유형 | `cuisine_slot_ratio` · `cuisine_slot_max` | 0.1 · 2 | 음식 유형 몫의 비율과 절대 상한. 0 이면 끔 |
...
| 레시피 쪽 음식 유형 | `recipe.cuisine_family` 는 09-17 규칙 배정으로 46,353건 중 28,604건(61.7%)이 찼고 나머지는 NULL 입니다. 배정 규칙이 태그 순회 순서에 따라 달라지는 결함이 있어(G-31) 재실행마다 일부 레시피의 계열이 바뀔 수 있습니다 | A 가 표본 정확도를 확인하고 결정론을 고친 뒤 `f_cuisine` 을 `PENDING_DATA_FEATURES` 에서 뺄 시점 |
