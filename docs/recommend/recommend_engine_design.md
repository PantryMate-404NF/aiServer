# 추천 코어 엔진 설계 명세 (Track B)

**정하는 것**: Pentry-Mate 추천 시스템 코어 서빙 파이프라인(Track B)의 구현 명세. 데이터 계약, 알고리즘과 수식, DDL, 소스 배치, 단계별 검증 계획

**적용 대상**: 파트 B(추천 엔진) 구현자와 AI 코딩 에이전트. 파트 A·C 와 백엔드는 2절과 4절의 접점만 봅니다
...
시스템 개발과 검증 파이프라인은 3단계 엔지니어링 레이어로 분리하여 구축합니다.
...
- Layer 1: 실제 데이터가 없는 개발 초기 단계에서 Mock 데이터셋을 기반으로 Retrieval, Scorer, Penalty, Re-ranking 코어 알고리즘을 독립 구현합니다.
- Layer 2: 백엔드(BE)의 가변적인 요청을 흡수하는 스키마 어댑터를 배치하고, Track A 의 `recipe_feature` 실데이터와 PostgreSQL `intarray` 인덱스를 바인딩하며, 서빙 로그 적재 파이프라인을 개통합니다.
- Layer 3: 42개 계약 검증 테스트 통과, 동시 부하 상황에서의 SLA(p95 < 58ms) 충족 여부, 유저 피드백 반영 루프를 최종 종단 검증합니다.

### 1.2 엔진 코어 4대 불변 원칙

1. **Zero-Drop 정규화**: 측정할 수 없는 피처(데이터 미입력, 임박 재료 부재 등)는 0점으로 처리하지 않고 점수 계산의 분자와 분모 가중치 합에서 동시에 제외합니다.
2. **곱연산 감점 (Penalty Multiplication)**: 최근 노출과 조리 이력에 대한 감점은 뺄셈이 아닌 곱셈으로 적용하여 고득점 요리의 독점을 원천 차단합니다.
3. **알레르기 100% 하드컷**: 알레르기 유발 식재료는 감점 대상이 아니며, Stage 1 Retrieval 단계에서 SQL 조건으로 완전 제외합니다.
4. **맛 벡터 코퍼스 중심화 (Mean Centering)**: 맛 코사인 유사도 계산 전, `feature_stats` 테이블에 기록된 코퍼스 평균 벡터를 레시피와 사용자 벡터 양쪽에서 동일하게 차감합니다.

---

## 2. 데이터 계약과 스키마 어댑터

백엔드와 클라이언트의 필드 변경 요구가 추천 랭킹 코어를 파괴하지 않도록 Pydantic v2 기반의 어댑터 패턴(Tolerant Reader)을 적용합니다.
...
### 2.1 API 입출력 스키마 (`src/features/recommend/schema.py`)

구현에서 달라진 점(고정 필드 `meta`, 이벤트 타입과 시각의 검증 타입)은 작업 기록 D-07, D-08 에 있습니다.

**요청 스키마 `RecommendRequest`**
...
46,552건의 레시피 풀에서 사용자 인벤토리와 호환되는 500개 후보군을 15ms 이내에 추출합니다(예시값, 실제 데이터로 대체 필요).
...
**3단계 완화 폴백**: 후보군이 20건 미만일 경우 부족수 완화($k = 2 \rightarrow 3 \rightarrow 4$), 대체재 확장, 전체 인기순 폴백을 단계적으로 실행하며, `meta.degraded = True` 상태로 200 OK 를 보장합니다. 구현은 조회 범위와 인기순 폴백의 조리시간 상한을 다르게 다룹니다(작업 기록 A-10, D-02, 검증 기록 X-04).

### 3.2 Stage 2: 5블록 Zero-Drop 랭킹 스코어러와 감점기

17개 피처를 5개 독립 블록으로 그룹화하여 가중합을 계산하며, 결측 블록은 분자와 분모에서 동시에 제외합니다.
...
단, $|P_{u,\text{D-3}}| = 0$ 이면 블록 계산에서 제외합니다.
...
단, $\boldsymbol{\mu}$ 는 `feature_stats` 테이블에서 조회한 코퍼스 3축 평균 벡터입니다. 축 순서는 (매움, 짠맛, 단맛)이며 `recipe_feature.flavor_vec` 6축의 앞 3축과 같습니다(작업 기록 D-16). 구현은 코사인을 0~1 로 옮기고 평균 부근의 사용자에게는 신뢰도를 낮춥니다(D-09, 검증 기록 X-02).

$S_{\text{quality}}$ (품질과 대중성, $w_4 = 0.15$)

$$S_{\text{quality}} = 0.6 \cdot \text{popularity\_score} + 0.4 \cdot \text{quality\_score}$$

$S_{\text{ctx}}$ (조리시간과 환경 적합도, $w_5 = 0.10$)

$$S_{\text{ctx}} = 1.0 - \max\left(0, \frac{T_r - T_{\max}}{T_{\max}}\right)$$

구현은 이 블록에 선호 요리군 일치를 평균으로 더합니다(검증 기록 X-06).
...
### 3.3 Stage 3: Re-ranking 과 20% 비페르소나 탐색 정책

최종 Top-20 서빙 결과 중 16개(80%)는 개인화 맞춤형으로, 4개(20%)는 취향 외 영역 탐색형으로 배분합니다. 구현은 개인화 슬롯을 먼저 채우고 남은 후보에서 탐색하며, 낯선 후보가 부족하면 탐색 수를 줄입니다(작업 기록 D-01, 검증 기록 X-05).
...
- 사용자가 온보딩에서 선택하지 않은 요리군(`preferred_cuisines` 외)과 미경험 맛 영역 중 `quality_score` 상위 후보를 추출합니다.
- 2건은 카테고리 사후 분포 $\text{Beta}(\alpha_c, \beta_c)$ 기반 Thompson Sampling, 2건은 균등 무작위로 추출합니다.
- 각 탐색 아이템의 노출 확률 역수($1 / P(r)$)를 `propensity_scores` 로 산출하여 로깅합니다(오프라인 학습 Position Bias 보정용).
- 4개 아이템을 1~20위 구간에 무작위 분산 배치(`random.sample(range(top_k), 4)`)하며 `is_exploration = True` 로 마킹합니다.

### 3.4 동적 페르소나 피드백 루프 수식

이 절의 EMA 와 선형 전이는 2026-09-11 2차 회의로 대체됐습니다. 현재 구현은 고른 음식의 6축 평균을 사전 취향으로 두고, 시각이 있는 이벤트를 반감기 감쇠와 주기 친화도로 가중 평균해 합칩니다. 근거는 `../decisions/2026-09-11_taste_persona_from_picks_with_time_decay.md`, 일반 설명은 `recommend_engine_how_it_works.md` 입니다. 아래는 그때의 계획입니다.

조리와 클릭 이벤트 발생 시 유저의 행동 풍미 벡터를 지수 이동 평균(EMA)으로 갱신하고 콜드·웜 전이를 처리합니다.
...
## 4. 데이터베이스 DDL 과 관측 인터페이스

### 4.1 테이블 DDL (`src/features/recommend/tables.py`)

A 트랙의 `deploy/init/02_schema.sql` 에 같은 이름의 테이블이 이미 있으며 `user_vector.taste_vec` 은 6축입니다. 소유와 축 수는 회의 안건 G-01, G-04 입니다.
...
1. 설정 지문(`config_fingerprint`): 가중치와 감점 계수 조합의 해시값을 생성하여 서빙 로그에 바인딩합니다. Track C 의 추천 디버거 화면이 당시 점수 가중치를 복원하는 유일한 단서가 됩니다.
2. `/health` 관측 카운터: `reco_served_total`, `reco_failed_total`, `reco_degraded_total` 카운터를 관리하고 엔드포인트에 노출합니다.
3. 특성 리포트 생성(`evaluation/feature_report.py`): 17개 특성별 min, median, max, null 비율을 계산하는 리포트 생성기를 배치하여 Track C 에 제공합니다.

---

## 5. 소스 코드 배치와 모듈 책임

저장소 규칙에 따라 모든 코드는 `src/features/recommend/` 하위에 둡니다.
...
구현에서 달라진 배치(`stage.py`, `engine/context.py`, `engine/feedback.py`, SQL 의 `repository.py` 배치)는 작업 기록 D-03, D-15, D-17 에 있습니다.

---

## 6. 단계별 구현과 검증 계획

각 단계의 구현 작업과 검증 항목입니다. 결과는 이 문서에 표시하지 않고 검증 기록에 적습니다. 검증 명령이 참조하는 `make` 대상은 저장소에 아직 없습니다(작업 기록 P-05). 소요 일수는 착수 시점의 추정입니다(예시값, 실제 데이터로 대체 필요).

### 6.1 Step 0: 데이터 계약과 가상 환경 구축 (Day 1)

**구현 작업**

- [ ] `src/features/recommend/schema.py` Pydantic v2 스키마 작성
- [ ] `scripts/generate_mock_fixtures.py` 작성과 12인 가상 인벤토리 생성
- [ ] 더미 응답을 서빙하는 기본 라우터 골격 연결

**검증 항목**

| 검증 ID | 검증 시나리오 | 통과 기준 | 검증 명령 |
|---|---|---|---|
| TC-0-1 | 가변 페르소나 입력 파싱 | 문자열 "4인 가구" 유입 시 `household_size=4` 추출 | `uv run pytest tests/unit/recommend/test_schema.py -k test_persona` |
| TC-0-2 | 규약 외 임의 필드 유입 | 에러 없이 임의 필드 무시 (`extra="ignore"`) | `uv run pytest tests/unit/recommend/test_schema.py -k test_extra` |
| TC-0-3 | 12인 픽스처 유효성 | 12개 Mock JSON 파싱 성공 | `uv run pytest tests/unit/recommend/test_schema.py -k test_fixtures` |

### 6.2 Step 1: 관측과 피드백 로깅 배관 개통 (Day 2)

**구현 작업**

- [ ] `tables.py` 선언과 DB 테이블 마이그레이션 (`recommendation_log`, `event_log`, `user_vector`)
...
| 검증 ID | 검증 시나리오 | 통과 기준 | 검증 명령 |
|---|---|---|---|
| TC-1-1 | 서빙 로그 적재 확인 | 1회 서빙 요청 시 `recommendation_log` 에 1건 저장 | `make log-test` |
| TC-1-2 | 로깅 예외 격리 검증 | DB 연결 강제 차단 상태에서도 서빙 응답 정상 200 OK | `uv run pytest tests/unit/recommend/test_logging.py` |
| TC-1-3 | 관측 카운터 노출 | 서빙 시 `/health` 의 `reco_served_total` 1 증가 | `curl -s http://localhost:8000/health \| grep reco_served` |

### 6.3 Step 2: Stage 1 Retrieval 과 3단계 완화 폴백 구현 (Day 3)

**구현 작업**

- [ ] `engine/candidate.py` 내 PostgreSQL `intarray` 쿼리 바인딩 작성
- [ ] 알레르기 유발 식재료 100% 하드컷 검증
- [ ] 후보군 20건 미만 시 $k$ 완화와 인기순 폴백 구현

**검증 항목**

| 검증 ID | 검증 시나리오 | 통과 기준 | 검증 명령 |
|---|---|---|---|
| TC-2-1 | 알레르기 차단 검증 | 결과 500개 중 알레르기 식재료 포함 레시피 0건 | `uv run pytest tests/unit/recommend/test_candidate.py -k test_allergy` |
| TC-2-2 | 3단계 완화 폴백 검증 | 후보 부족 인벤토리 유입 시 `meta.degraded=True` 와 20건 반환 | `uv run pytest tests/unit/recommend/test_candidate.py -k test_fallback` |
| TC-2-3 | 집합 쿼리 지연시간 | 후보 500개 추출 레이턴시 p95 < 15ms | `make smoke` |

### 6.4 Step 3: Stage 2 랭킹 스코어러와 감점기 구현 (Day 4~5)
...
| 검증 ID | 검증 시나리오 | 통과 기준 | 검증 명령 |
|---|---|---|---|
| TC-3-1 | Zero-Drop 정규화 검증 | 임박 재료 없을 시 $w_2$ 가 분모에서 제외되어 점수 왜곡 없음 | `uv run pytest tests/unit/recommend/test_rank.py -k test_zero_drop` |
| TC-3-2 | 중심화 미적용 감지 | 코퍼스 평균 미차감 시 테스트 강제 실패 (0.767 쏠림 방지) | `uv run pytest tests/unit/recommend/test_rank.py -k test_centering` |
| TC-3-3 | 감점 곱연산 검증 | 최근 14일 내 조리 레시피는 최종 스코어가 정확히 50% 감쇄 | `uv run pytest tests/unit/recommend/test_penalty.py` |

### 6.5 Step 4: Stage 3 Re-ranking 과 20% 탐색 정책 구현 (Day 6~7)

**구현 작업**

- [ ] `engine/rerank.py` 내 MMR 자카드 다양성 필터 구현 ($\lambda = 0.7$)
- [ ] 4개 비선호 탐색 슬롯 생성 (Thompson Sampling 2건 + 균등 무작위 2건)
- [ ] 탐색 아이템 위치 무작위화와 IPS 역확률 계산 로깅
...
| 검증 ID | 검증 시나리오 | 통과 기준 | 검증 명령 |
|---|---|---|---|
| TC-4-1 | 탐색 슬롯 비율 검증 | 20개 결과 중 `is_exploration=True` 아이템 정확히 4개 | `uv run pytest tests/unit/recommend/test_rerank.py -k test_exploration_count` |
| TC-4-2 | 위치 무작위화 검증 | 100회 실행 시 탐색 슬롯의 평균 인덱스 분산 확인 (고정 순위 금지) | `uv run pytest tests/unit/recommend/test_rerank.py -k test_random_positions` |
| TC-4-3 | 사유 문구 유효성 | 결측값 없이 자연스러운 한국어 조사 바인딩 완료 | `uv run pytest tests/unit/recommend/test_explain.py` |

### 6.6 Step 5: 실서빙 조립과 피드백 루프 연동 (Day 8~9)

**구현 작업**

- [ ] `service.py` 와 `router.py` 엔드투엔드 파이프라인 연결 (난수 제거)
- [ ] `POST /v1/events` 엔드포인트 구현과 유저 취향 벡터 EMA 감쇠 갱신
...
| 검증 ID | 검증 시나리오 | 통과 기준 | 검증 명령 |
|---|---|---|---|
| TC-5-1 | 난수 점수 잔존 여부 | 동일 입력에 대해 결정론적 스코어 출력 확인 | `uv run pytest tests/unit/recommend/test_service.py -k test_deterministic` |
| TC-5-2 | 피드백 감쇠 갱신 | 조리 이벤트 20회 누적 시 행동 풍미 반영 비율 100% 도달 | `uv run pytest tests/unit/recommend/test_feedback.py` |
| TC-5-3 | 특성 리포트 무결성 | 17개 특성의 결측률과 통계치가 정상 산출됨 | `uv run python src/features/recommend/evaluation/feature_report.py` |

### 6.7 Step 6: 통합 종단 테스트와 레이턴시 검증 (Day 10~11)

**구현 작업**

- [ ] Track A `recipe_feature` 실데이터 연동 확인
- [ ] 42개 계약 검증 단언 테스트 통과
- [ ] Locust 기반 부하 테스트와 레이턴시 SLA 검증

**검증 항목**

| 검증 ID | 검증 시나리오 | 통과 기준 | 검증 명령 |
|---|---|---|---|
| TC-6-1 | 계약 검증 단언 테스트 | 42개 케이스 100% 통과 | `make contract` |
| TC-6-2 | 실데이터 무결성 검증 | 비정상 결측치와 테이블 스키마 오류 0건 | `make feature-test` |
| TC-6-3 | 서빙 지연시간 SLA | 100회 동시 요청 상황에서 단일 요청 p95 < 58ms (SLA 300ms) | `locust -f tests/load/locustfile.py --headless -u 10 -r 2 --run-time 1m` |

---

## 7. 정량 목표 지표와 실측 기록표

배포 전 오프라인 평가 하네스와 실측 테스트를 통해 아래 지표를 달성하고 기록합니다. 목표치는 검증 전 값입니다(예시값, 실제 데이터로 대체 필요). 실측치는 검증 기록에 적습니다.

| 평가지표 | 최저 기준치 | 제안 모델 목표치 |
|---|---|---|
| 서빙 지연시간 (p95) | < 300ms | < 58ms |
| NDCG@10 (정밀도) | >= 0.85 | 0.926 |
| Recall@20 (재현율) | >= 0.60 | >= 0.60 |
| 카탈로그 커버리지 | >= 15.0% | >= 15.0% |
| 목록 다양성 (ILD) | >= 0.95 | >= 0.95 |

### 7.1 최적화 가중치 착수 파라미터 (Bradley-Terry 학습 후 확정)

| 파라미터 | 항목 | 착수 값 |
|---|---|---|
| `w_match` | 재료 충족도 | 0.29 |
| `w_expiring` | 소비기한 소진 | 0.15 |
| `w_taste` | 3축 미각 적합도 | 0.31 |
| `w_quality` | 품질과 대중성 | 0.15 |
| `w_ctx` | 상황과 시간 적합도 | 0.10 |
| `config_fingerprint` | 생성된 MD5 해시값 | 실행 시 생성 |

학습 후 실측치는 검증 기록에 적습니다.

---

## 8. 최종 품질 검증 명령

이 명세의 모든 단계를 완료한 후, 커밋 전 아래 통합 명령어를 실행하여 전수 통과 결과를 확보합니다.
