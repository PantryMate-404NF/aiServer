...
| 타 파트 안건 | G-01~G-19. **G-01~G-05, G-07 결정 완료**(9.5), **G-08·G-11 은 병합에서 해소**(9.6). 남은 것 G-06, G-09·G-10, G-12~G-19 |
| 내부 안건 | N-01~N-13. 회의 결과에 걸린 것 3건(N-02, N-03, N-08), 나머지는 B 가 바로 처리 |
...
| G-18 | `make contract` 가 조용히 중단됨 | `.env` 가 비어 있으면 `create_app()` 이 `ValidationError` 를 내는데 `try` 가 `ImportError` 만 잡습니다. 59번째 체크에서 멈추고 뒤쪽 계약(S1 DB·탐색·온보딩·session_id)은 실행되지 않으며 **마지막으로 보이는 줄이 통과 표시라 눈으로는 통과처럼 읽힙니다**(F-30) | (a) `except Exception` 으로 넓히고 실패로 세기 (b) 종단 블록을 `.env` 없이도 도는 설정으로 분리 (c) `Makefile` 이 검사용 환경변수를 세움 | (a)+(c). 지금은 종료 코드로만 알 수 있고 사람이 출력을 읽으면 놓칩니다 | A | **(a) 적용**. 종단 블록 두 곳이 이제 실패로 셉니다 — 빈 `.env` 에서 92통과 2실패 종료코드 1. (c) `Makefile` 이 검사용 환경변수를 세우는 것은 A 판단 |
| G-19 | `/v1/health` 의 `redis` 필드 | 저장소 어디에도 redis 클라이언트가 없는데 `HealthOut.redis` 기본값이 True 라 항상 참으로 나갑니다(F-31). `db` 는 실제 확인으로 고쳤습니다(F-24) | (a) 필드를 뺌 (b) redis 를 실제로 붙이고 확인 (c) 그대로 둠 | (a). 근거 없는 참은 없느니만 못합니다 | A | 미정 |
...
| N-13 | `f_time_fit` 의 재정의 | 후보 조회가 조리시간 상한을 이미 걸러서 살아남은 후보는 전부 1.0 입니다. 가중치 0.03 이 순위를 못 바꿉니다(F-27) | 상한 대비 여유분을 재는 쪽으로 바꾸면 순위가 달라집니다. 값을 바꾸는 결정이라 W3 쌍대비교 학습(N-04·N-05)에서 `f_pantry_use`·`f_quality` 의 가중치와 함께 정합니다 | 대기 |
