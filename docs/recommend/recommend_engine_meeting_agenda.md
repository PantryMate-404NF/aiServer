...
| G-08 | A `tests/conftest.py` 의 `collect_ignore_glob` | `unit/recommend/*.py` 가 B 테스트 12 파일을 수집에서 뺌(P-13). A 가 파일명 명시로 고치겠다고 함 | (a) A 의 단독 실행 스크립트를 파일명으로 명시 (b) A 스크립트를 `scripts/` 로 이동 | (b) 가 02의 3.3 에 맞음. 급하면 (a) | A | **(a) 적용** (D-26). 글로브가 B 검사 63건과 `integration/test_receipt_pipeline.py` 까지 뺐습니다. (b) 로 갈지는 A 판단 |
...
| G-14 | Windows 에서 `make` 검사가 죽음 | A 의 검사 스크립트가 `✓` 를 출력하는데 Windows 기본 콘솔이 cp949 라 `UnicodeEncodeError` 로 끝납니다. 검사 실패가 아니라 출력 실패인데 종료 코드가 1 입니다 (E-11) | (a) `Makefile` 의 `PY` 에 `PYTHONIOENCODING=utf-8` 을 세움 (b) 스크립트에서 `sys.stdout.reconfigure` (c) 각자 환경변수 | (a). 한 곳에서 끝나고 `make data-gate` 를 그대로 쓸 수 있습니다 | A | 미정 |
