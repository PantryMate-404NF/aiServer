...
**버전**: 1.1.0 · **최종 수정**: 2026-09-18 · **작성자**: 파트 A (데이터)
...
A 쪽은 `enums.ALLERGEN_GROUPS` 로 정본을 세웠습니다. `tests/unit/recommend/test_allergen_vocab.py` 가 DDL 과의 일치를 대조합니다.

입구의 동작은 09-18 에 바뀌었습니다. 처음에는 모르는 값 하나에 요청 전체를 거부했는데, 그러면 알러지만 빠지는 것이 아니라 고른 음식과 맛 척도까지 함께 사라지고 그 사용자는 알러지 0행이 되어 차단이 아예 꺼집니다. 응답도 본문 없는 400 이라 어느 값이 문제였는지 부르는 쪽이 알 수 없었습니다. 거부보다 나쁜 결과입니다.

지금은 해석되는 것만 적용하고, 못 한 것은 `OnboardingOut.unmapped_allergens` 로 돌려줍니다. 조용히 버리지 않는 것이 핵심입니다. 화면이 쓰는 한글 표기도 함께 받습니다 — 표시 대상 19종 가운데 13종은 그룹으로 막고, 나머지 6종(돼지고기·닭고기·쇠고기·오징어·토마토·아황산류)은 막을 방법이 없다고 명시해 돌려줍니다.

오징어를 `shellfish` 로 보내지 않는 것이 중요합니다. 그 그룹에 두족류가 한 종도 없어, 넣으면 사용자는 막혔다고 믿는데 두족류 레시피 1,141건 중 841건이 그대로 지나갑니다. 과소차단이라 거부보다 위험합니다.
