...
| `recipe.cuisine_family` | 46,353건 전수 NULL | 규칙 배정 준비 완료 (61.7%) | 6절 참조. 아직 반영 전입니다 |
...
```text
레시피 46,353건 · 배정 28,621건 (61.7%)
korean 21,055 · western 5,293 · japanese 846 · chinese 794 ·
asian_other 589 · fusion 44
```

순서가 규칙의 절반입니다: **① 태그 ② 비한식 제목 ③ 한식 제목 ④ 시그니처 재료.** 비한식을 먼저 보는 이유는 `김치파스타` 가 양식이어야 하기 때문이고, 한식 제목을 재료보다 먼저 보는 이유는 다진마늘·참기름·고춧가루가 거의 모든 레시피에 있어(실측 27,359건) 재료를 먼저 보면 **전부 한식이 되기** 때문입니다.

**못 정하면 NULL 입니다.** 한식으로 폴백하면 100% 가 되지만 그 17,732건에 분류 못 한 양식·퓨전이 섞여 있고, 한식으로 박으면 그 사용자가 영영 못 만납니다. 비한식 계열은 폴백을 켜도 숫자가 같습니다.

시드가 한때 `southeast_asian`(열거형에 없음)과 `italian`(계열이 아니라 세분 코드)을 뱉었습니다. 그대로 넣으면 온보딩이 고른 값과 영영 안 만나는데 INSERT 는 성공합니다. `Rules.check()` 이 시작부터 거부하게 했습니다.

**아직 DB 에 안 썼습니다.** 표본 120건의 정확도를 사람이 확인한 뒤 반영합니다. 미리보기에서 이미 `카레` 키워드가 한국식 카레를 `asian_other` 로 보내는 것이 보였습니다.
