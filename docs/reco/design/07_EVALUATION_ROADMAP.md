...
> (`docs/01:2189` · `src/features/recommend/evaluation/threshold.py` 서두)
...
`src/features/recommend/evaluation/threshold.py::calibrate` 가 이미 이 패턴의 선례다. 목표를 만족하는 임계값이
...
| # | 무엇 | 어디서 | 근거 |
|---|---|---|---|
| 1 | **오프라인 랭킹 지표** — 요청 단위 pairwise 정확도(주) · NDCG@10 / Recall@20 / MAP@10 / MRR(보조) | `src/features/recommend/evaluation/metrics.py` (W5) | `docs/01 6-10` |
| 2 | **온라인 지표** — CTR@K · Save rate · **Cook conversion** · Dismiss rate · 폴백률 | `v_real_events` · `v_real_recommendations` 뷰 (`deploy/init/04_functions.sql`) | `docs/01 6-10` |
| 3 | **Guardrail** — Catalog Coverage · ILD · Novelty · Gini + D-5 의 재료 축 2종 | 동상 | `docs/01 6-10`, D-5 |
| 4 | **온라인 비교** — Team-Draft Interleaving (`src/features/recommend/engine/explore.py::interleave`) | W7 | `docs/01 5-7-2` |
...
```python
# src/features/recommend/evaluation/metrics.py — 모집단을 시그니처에 박는다
...
```python
# src/features/recommend/evaluation/split.py — 함수 2개, 불변식 2개
...
```sql
-- deploy/init/04_functions.sql — retrieve_candidates · retrieve_for_user
...
Team-Draft Interleaving (`docs/01 5-7-2`, `src/features/recommend/engine/explore.py::interleave`, `04` P1 #13 8h)은
...
| 필요한 것 | 현재 상태 |
|---|---|
| `RankedItem.team` | ✅ `src/features/recommend/stage.py:175` 에 존재 (API 응답 전용) |
...
> propensity 는 MC 200회로 계산해 로깅한다(`src/features/recommend/engine/serendipity.py`). 결정적 18칸의
...
(`src/features/recommend/engine/explore.py:31`)는 결정적 슬롯에 **1.0** 을 돌려주며, docstring 이 그 의미를
...
`src/features/recommend/evaluation/threshold.py::ThresholdResult.usable` 과 같은 형태다 — 예외로 리포트를 막지 않고,
...
> ⚠️ `retrieve_candidates`(`deploy/init/04_functions.sql`)가 `max_missing=2` 로 이미 자르므로
...
> 나올 가능성이 높다.
>
> **결론을 내지 말고 재라.** `scripts/reco/bench/` 에 붙이면 하루면 답이 나온다.
...
**③④⑤⑥⑦⑨⑩ 은 스키마에 반영됐다.** `deploy/init/02_schema.sql`·`03_indexes.sql`·`04_functions.sql`
직접 수정 후 재초기화 1회. 상세는 `docs/reco/decisions/2026-09-04_ddl_revision.md`.
...
그리고 🔑 **`recipe_feature` 행을 반드시 삭제** — `retrieve_candidates`(`deploy/init/04_functions.sql`)는
...
### ② `event_log` — ✅ **적용 완료** (`deploy/init/02_schema.sql`)

아래는 **제안이 아니라 현재 DDL 이다.** `deploy/init/02_schema.sql` 에서 확인했다.
...
> ✅ **완료 (09-03).** `position` 을 1-base 로 못박았다 — `src/features/recommend/schema.py` 가
...
```sql
-- deploy/init/04_functions.sql :: effective_expiry
...
```sql
-- deploy/init/03_indexes.sql — 기존 UNIQUE 를 대체한다. 재구매하면 새 행이 쌓인다.
...
| **`src/features/recommend/evaluation/ope.py` 4종 추정량 (IPS·SNIPS·DM·DR)** | 100% 소급 가능. 지금 필요한 것은 추정량이 아니라 `ess(w)=(Σw)²/Σw²` 계산기 하나와 재구성 재료 로깅뿐 | W7 이후 |
...
| ~~**LLM 추천 이유 생성**~~ → 🔄 **트랙1 한정으로 뒤집혔다 (D-19)** | 원래 이유: `src/features/recommend/engine/reason.py` 템플릿 15종이 종결형/연결형 2벌 + 받침 조사 처리까지 완성. 0ms · 환각 0. **냉장고 서비스에서 없는 재료를 언급하는 것은 치명적**. → **09-03 D-19 가 트랙1(오늘의 추천)을 사전 계산으로 만들기로 하면서 그 트랙의 사유는 LLM 이 쓴다.** 온라인 트랙2 는 템플릿 그대로다 | ✅ **트랙1 은 지금** (배치) · 트랙2 는 — |
...
| ~~**`docs/reco/design/06_INFRA_SPEC.md` 를 그대로 인용**~~ → ✅ **09-03 에 06 이 고쳐졌다** | 지적 내용: `event_log 397MB` · `recommendation_log(시뮬) 286MB` · **"시나리오 A 에서 로그가 디스크의 77%"** 가 전부 **시뮬 5,000유저 전제**라 시뮬레이터 컷(`04 §2`) 이후 무효다. → **06 §2 가 09-03 에 취소선 처리되고 실유저 100명 기준으로 재산출됐다** (`event_log` 약 3MB). **인용해도 되는 상태다** |
