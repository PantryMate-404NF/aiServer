...
**모든 도메인이 같은 구성을 가집니다.** 다른 도메인으로 옮겨가도 파일 이름이 같아 탐색 비용이 없습니다.

| 위치 | 개수 | 내용 |
|---|---|---|
| 도메인 루트 | `.py` 6개 | `__init__`, `router`, `schema`, `service`, `repository`, `tables` |
| `pipeline/` | 도메인마다 다름 | 처리 단계. 도메인 고유 로직이 모입니다 |
...
| `pipeline/` | 이 도메인의 처리 단계 | 단계별 모듈 | 다른 도메인 import, 흐름 조립 |
