...
**정하는 것**: 파일을 어디에 두는가. 폴더 배치, 네이밍, 분리 기준, import.

**적용 대상**: aiServer 소스 트리(`src/`)

**버전**: 1.3.0 · **최종 수정**: 2026-09-18 · **작성자**: 김민경
...
**축 사이 의존은 한 방향으로만 흐릅니다**(MUST). `recommend` 는 `ingest` 에서 `engine` 으로, `engine` 에서 `evaluation` 으로 흐릅니다. 뒤 축은 앞 축의 **순수 함수와 계약 타입**을 import 할 수 있습니다(MAY). 평가가 엔진의 점수·유사도 함수를 그대로 쓰는 것이 그 예이며, 같은 계산을 두 벌 두면 한쪽이 조용히 어긋나기 때문입니다. 앞 축이 뒤 축을 호출하는 것(엔진이 평가를, 수집이 엔진을)은 금지합니다(MUST NOT). 어느 축도 다른 축의 DB 접근이나 흐름 조립을 부르지 않으며, 축을 잇는 흐름은 `service.py` 에만 둡니다. 이 방향이 깨지면 축을 나눈 의미가 사라집니다.
...
`docs/` 아래의 파일명, 배치, 헤더, 본문 형식은 [04_DOCUMENTATION_RULES.md](04_DOCUMENTATION_RULES.md) 가 정합니다. 이 문서에 복사하지 않습니다(MUST NOT).
