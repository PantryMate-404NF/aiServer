...
| ID | 내용 | 위치 |
|---|---|---|
| C-01 | 인기순 폴백은 조리시간 상한 무시. 알레르기 컷만 유지 | `candidate.select_candidates` |
| C-02 | 탐색 0개 조건: 후보 수 == `top_k`, 비선호 요리군 후보 없음, `cuisine None` 만 남음. `preferred_cuisines` 비면 요리군 있는 전부가 탐색 대상 | `rerank.pick_exploration`, `_is_novel` |
...
| I-02 | Track A → 엔진 | `RecipeCandidate` | `catalog.json` 120건 | `repository.fetch_candidates` 가 `recipe_feature` 행 변환. 컬럼: `recipe_id`, `title`, `essential_ids`, `all_ids`, `flavor_vec`(3), `popularity_score`, `quality_score`, `cook_minutes`, `cuisine`, `product_ids` |
| I-03 | Track A → 엔진 | `CorpusStats` | conftest 가 픽스처에서 계산 | `feature_stats`: `flavor_mean`(3), `ingredient_idf`(id→float), `ingredient_names`(id→str) |
| I-04 | DB 이력 → 엔진 | `UserHistory` | 빈 값 / 테스트 지정 | `user_vector`(행동 취향, 이벤트 수) · 알레르기 코드→ID(T-06) · 기피 재료(P-07) · 최근 7일 노출(`recommendation_log`) · 최근 14일 조리(`event_log`) · 대체재(P-08) · `cuisine_priors`(T-08) |
| I-05 | 엔진 → DB | `service.ServingLog` | 테스트 모양 검사 | `repository.log_serving_result` → `recommendation_log`. `candidates_features` ← `ScoredCandidate.blocks/base_score/score`, `propensity_scores` ← `ServedItem.propensity` |
...
| ID | 가정 | 어긋나면 고칠 곳 |
|---|---|---|
| A-01 | `flavor_vec` 축 `(spicy, sweet, salty)`, 0~1 척도. `user_vector` 기본 `{0.5,0.5,0.5}` 동일 척도 | `schema.FLAVOR_AXES`, `TastePreference.as_vector`, `feedback.DEFAULT_BEHAVIOR_VEC` |
...
| A-05 | 알레르기 코드군 19종 ↔ 재료 ID 매핑 존재 | T-06. 픽스처 `allergen_groups` 대역 |
...
| A-10 | **후보 조회 범위**: `select_candidates` 는 넘겨받은 풀 안에서만 완화. 계획서 SQL 처럼 k=2 + `LIMIT 500` 이면 완화 후보가 없어 폴백 빈손 | T-05: k=4 로 한 번에 조회(정렬 부족수 오름차순이라 상위 500 이 k=2 포함) 또는 단계별 재조회. 인기순 폴백은 별도 조회 |
