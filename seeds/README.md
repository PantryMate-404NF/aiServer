...
### 6. 가공품의 알러지 예외 규칙은 배포 이후로 미룬다 (2026-09-20)

성분은 제조사마다 다른데 **우리에게 성분 데이터가 없다.** 크롤한 상품의
`specifications` 와 `detail_text` 는 전량 비어 있다. 그래서 판단이 요리 상식에
기대게 되고, 예외를 하나 인정하면 비슷한 것이 줄줄이 따라온다.

지금까지 정한 것은 하나뿐이다.

| 재료 | 판단 | 근거 |
|---|---|---|
| `돈까스소스` | `gluten` | 우스터소스 계열이라 소맥분이나 양조간장이 든다 |

**기본값은 "있다고 본다"이고, 글루텐 프리 표기가 있는 제품만 예외로 본다.**
알러지는 과차단이 과소차단보다 안전한 쪽이기 때문이다.

아래는 **배포 이후에 다시 볼 것**이다. 지금 손대지 않는다.

| 재료 | 레시피 | 미룬 이유 |
|---|---|---|
| `스테이크소스` | 135건 | 돈까스소스와 같은 우스터소스 계열 |
| `케첩` | 1,646건 | 보통 밀이 안 들어가지만 제품차가 있다 |
| `머스타드`·`연겨자`·`와사비` | 744건 | 분말 제품에 밀 전분이 드는 경우가 있다 |
| `쯔유`·`어간장` | — | 간장 계열이라 `soy` 검토 대상이다 |
| `햄`·`소시지`·`베이컨` | 2,690건 | 결착제로 밀이 드는 제품이 있다 |
| `건포도` 등 건과일 | — | 아황산류. 우리 그룹에 자리가 없다 |

**전분은 글루텐이 아니다.** 옥수수·감자 전분에는 없고, 글루텐은 밀가루나
양조간장에서 온다. 판단할 때 둘을 섞지 않는다.
