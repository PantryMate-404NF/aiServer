...
class Smoke:
    def __init__(self, cur, n_recipes: int, via_python: bool = False):
        self.cur = cur
        self.n = n_recipes
        #: S1 검증. True 면 같은 케이스를 features.recommend.repository.retrieve() 경로로 다시 돌린다.
    ...
    def cands_for(self, uid, max_missing=2, minutes=None):
        """features.recommend.repository.retrieve() 경로. (Candidate, title) 짝을 돌려준다.
        ...
        """
        from features.recommend.repository import retrieve

        self.cur.connection.commit()
        if not self._titles:
            self.cur.execute("SELECT id, title FROM recipe WHERE source = %s", (SRC,))
            self._titles = dict(self.cur.fetchall())
        return [
            (c, self._titles[c.recipe_id])
            for c in retrieve(uid, max_missing, minutes, CORRECTNESS_LIMIT, include_test=True)
            if c.recipe_id in self._titles
        ]  # 합성분만. 크롤 4.6만 건 제외
    ...
    def run_correctness(self):
        ...
        # ── A3. max_missing 필터 ────────────────────────────────
        self.check(
            "A3 max_missing=1 이면 김치찌개 제외",
            "김치찌개" in self.titles_for(u, max_missing=1),
            False,
        )
        self.check(
            "A3 max_missing=2 이면 김치찌개 포함",
            "김치찌개" in self.titles_for(u, max_missing=2),
            True,
        )
        ...
        self.check(
            "A4 직접 지정 — 두부 레시피 전부 차단",
            {"김치찌개", "두부조림"} & self.titles_for(a_direct),
            set(),
        )
        ...
        self.check(
            "A6 컬럼 전용 그룹 — 메밀전병 차단 (카테고리로는 못 잡음)",
            "메밀전병" in self.titles_for(a_grp),
            False,
        )
        ...
        self.check(
            "A7 알러젠이 고명에만 있어도 차단 (all_ids 검사 증명)",
            "잣죽" in self.titles_for(a_nut),
            False,
        )
        ...
        self.check(
            "A8 아몬드 알러지 → 같은 nut 그룹(호두·잣) 확산",
            {"호두강정", "잣죽"} & self.titles_for(a_sp),
            set(),
        )
        ...
        self.check(
            "A9 cook_minutes NULL 은 시간 필터로 버려지지 않는다",
            "시간미상요리" in self.titles_for(u, minutes=10),
            True,
        )
    ...
    def run_python_layer(self):
        ...
        """
        print("\n[1b] S1 — features.recommend.repository.retrieve() 계약")
        ...
        self.check(
            "P4 cluster_id 필드 존재",
            all(c.cluster_id is None or isinstance(c.cluster_id, int) for c, _ in pairs),
            True,
        )

        # P5. 부족 0 건은 missing_ids 가 NULL 로 올 수 있다 → [] 로 정규화.
        self.check(
            "P5 missing_ids NULL→[]", all(isinstance(c.missing_ids, list) for c, _ in pairs), True
        )

        # P6. 풀 재사용 — N번 호출해도 커넥션이 안 늘어난다.
        from features.recommend.repository import retrieve

        self.cur.execute("SELECT count(*) FROM pg_stat_activity WHERE datname = current_database()")
    ...
    def run_latency(self):
        ...
        for i in range(self.n):
            ...
            rows_f.append(
                (
                    ess,
                    sorted(set(ess + extra)),
                    len(ess),
                    random.random(),
                    random.choice([10, 20, 30, 45, 60, None]),
                )
            )
        # psycopg3 의 executemany 는 스스로 파이프라인으로 묶어 보낸다.
        # psycopg2 의 execute_values(page_size=...) 가 하던 일이 기본 동작이다.
        ...
        self.cur.executemany(
            """INSERT INTO recipe_feature
                 (recipe_id, essential_ids, all_ids, category_ids, n_essential, n_total,
                  flavor_vec, popularity_score, quality_score, cook_minutes, feature_version)
               VALUES (%s, %s::int[], %s::int[], %s::int[], %s, %s,
                       %s::real[], %s, %s, %s, %s)
        ...
        ok = p95 < 300
        print(
            f"  {'✓' if ok else '✗'} p95 < 300ms 목표 {'달성' if ok else '미달 → 플랜 B(Redis 역색인) 검토'}"
        )
        if ok:
            self.passed += 1
        else:
        ...
        if True:  # 선택도 6% 수준이면 Seq Scan 이 실제로 더 싸다(실측). 강제하지 않는다.
            print(
                f"  · 실행 계획: {'Bitmap Index Scan(GIN)' if used_gin else 'Seq Scan'} "
...
def main():
    ...
    ap.add_argument(
        "--via-python",
        action="store_true",
        help="S1 검증 — 같은 케이스를 features.recommend.repository.retrieve() 로 돌린다",
    )
    ap.add_argument(
        "--url",
