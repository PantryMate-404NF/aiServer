"""P3 매칭 캐스케이드 회귀 검증 (설계 4-4).
    ...
    .venv/bin/python -m tests.unit.recommend.test_match
...
check(
    f"사전 로드 — 재료 {len(d.names)} · alias {len(d.aliases)}",
...
for q, want_name, want_method in [
    ...
    r = match(q, d)
    check(
        f"{want_method.value:<5} {q} → {want_name}",
        r.ingredient_name == want_name and r.method == want_method,
    )

# ── 조회 키 정규화 — P1 이 띄어쓰기를 떼지 않는다 ────────
check("띄어쓰기가 있어도 매칭된다 ('대 파' → 대파)", match("대 파", d).ingredient_name == "대파")
check(
    "수식어 + 띄어쓰기 조합 ('국내산 대파' → 대파)",
...
check(
    "🔴 건포도 를 포도 로 합치지 않는다 (둘 다 사전 표제어)",
    match("건포도", d).ingredient_name == "건포도",
)
check(
    "🔴 생강 을 강 으로 부수지 않는다 (2음절 미만 잔여 금지)",
    match("생강", d).ingredient_name == "생강",
)
for a, b in [("참기름", "들기름"), ("진간장", "양조간장"), ("찹쌀가루", "쌀가루")]:
    ra, rb = match(a, d), match(b, d)
    check(f"🔴 {a} 와 {b} 가 서로 다른 재료로 남는다", ra.ingredient_id != rb.ingredient_id)
...
check("mention 과 distinct 를 따로 센다 (4-8)", cov.mention_total == 4 and cov.distinct_total == 3)
check(
    "mention 75% · distinct 67% (같은 표현 반복을 반영)",
