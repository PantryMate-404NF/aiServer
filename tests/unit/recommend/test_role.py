"""P4 역할 판정 회귀 검증 (설계 4-5).
    ...
    .venv/bin/python -m tests.unit.recommend.test_role
...
for txt, want_role, want_rule in [
    ...
    r = role_of(txt, d)
    check(f"{want_rule:<18} {txt} → {want_role.value}", r.role == want_role and r.rule == want_rule)
...
for txt in ["스팸 적당량", "모짜렐라치즈 적당량", "배추김치 약간", "버터 약간"]:
    r = role_of(txt, d)
    check(
        f"가공식품·유제품도 '약간' 이어도 essential — {txt}",
        r.role == IngredientRole.ESSENTIAL and r.rule == "R5_가드_주재료",
    )
...
for txt in ["대파 약간", "당근 적당량", "부추 약간"]:
    r = role_of(txt, d)
    check(
        f"채소는 '약간' 이면 optional 로 남는다 — {txt}",
...
check(
    "essential 이 아닌 3종은 Retrieval 에 동일하게 작용한다 (모두 제외)",
...
check("essential 비율은 판정된 것만으로 계산한다", abs(st.essential_ratio - 2 / 3) < 1e-9)

# ── 주재료 카테고리 상수가 실제 시드와 맞는가 ───────────────
cats = {(v.get("category_path") or "").split(".")[0] for v in d.meta.values()}
check(
    f"MAIN_CATEGORIES 가 실제 대분류에 존재한다 {MAIN_CATEGORIES}",
