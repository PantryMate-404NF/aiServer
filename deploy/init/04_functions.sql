...
SET search_path = reco, public AS $$
    ...
    WHERE  (p_include_test OR rf.feature_version NOT LIKE 'test-%')
      ...
      --
      --    🔴 09-17: 임계값을 0.3 에서 0 으로 조였다. **필수가 0건인 경우에 한해서다.**
      --    실측으로 n_essential=0 · n_total>0 인 2,739건이 이렇게 갈린다:
      --        미매칭 0건        59건   쌈장·초고추장·만능양념장 — 진짜로 양념만으로 만든다
      --        미매칭 있음     1,439건   비율이 0.3 이하라 통과하던 것들
      --        비율 > 0.3     1,241건   옛 임계값이 걸러내던 것들
      --    가운데 1,439건이 문제다. 필수를 **하나도** 못 찾았는데 미매칭이 남아 있으면
      --    "양념만으로 되는 요리" 가 아니라 "아직 못 읽은 재료 안에 필수가 있다" 는 뜻이다.
      --    그런데 아래 coverage 가 1.0 만점을 주고, ① 의 cardinality=0 가지가
      --    팬트리 필터까지 건너뛰어 **모든 사용자 상위에 올라온다.**
      --    비율 임계값은 n_essential ≥ 1 에서 "재료를 얼마나 놓쳤나" 를 재는 값이라
      --    필수가 0건인 자리에는 뜻이 다르다 — 거기서는 미매칭 1건도 판정 불가다.
